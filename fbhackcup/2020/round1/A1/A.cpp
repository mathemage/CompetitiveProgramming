...
void solve() {
  ...
  assert(SZ(L)==N);
  assert(SZ(H)==N);
  MSG(L); MSG(H); LINESEP1;

  ll mxLen=W+1;
  deque<ll> heightsOfIntPts(mxLen, H[0]);
//   MSG(SZ(heightsOfIntPts)); LINESEP1;

  ll P=2*(W+H[0]);
  ll result = P%MOD;
  MSG(P); MSG(result); LINESEP1;

  FOR(i,1,N-1) {
    if (L[i-1]+W < L[i]) {
      P+=2*(W+H[i]);
      for (auto & height: heightsOfIntPts) {
        height = H[i];
      }
    } else {
      FOR(pos, L[i-1]+W+1, L[i]+W) {
        heightsOfIntPts.push_back(H[i]);
        P+=2;  // top & bottom
      }
      MSG(SZ(heightsOfIntPts)); MSG(P);

      FOR(pos, L[i-1], L[i]-1) {
        heightsOfIntPts.pop_front();
      }
      assert(SZ(heightsOfIntPts)==mxLen);
      MSG(SZ(heightsOfIntPts)); MSG(P);

      P+=2 * max(0LL, H[i]-heightsOfIntPts.front());
      MSG(H[i]); MSG(heightsOfIntPts.front()); MSG(P);
