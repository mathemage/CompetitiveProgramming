/* ========================================
   ...
   * Last Modified : Tue 21 Sep 2021 09:06:05 PM CEST
...
void solve() {
  ...
  ll mnUnder, mxUnder, mnOver, mxOver;
  mnUnder=mxUnder=mnOver=mxOver=0;
  REP(i,N) {
    mnUnder+=max(0LL, X[i]-S[i]);
    mxUnder+=max(0LL, X[i]+Y[i]-S[i]);
    mnOver+=max(0LL, S[i]-X[i]-Y[i]);
    mxOver+=max(0LL, S[i]-X[i]);
  }

  ll result = 0LL;
  if (mnUnder>mxOver || mnOver>mxUnder) {
    result=-1;
  } else {
    result=max(mnUnder, mnOver);
  }
