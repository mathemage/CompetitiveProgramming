/* ========================================
   ...
   * File Name : A3.upsolve.cpp
   * Creation Date : 15-09-2021
   * Last Modified : Wed 15 Sep 2021 12:50:00 AM CEST
   * Created By : Karel Ha <mathemage@gmail.com>
   * URL : https://www.facebook.com/codingcompetitions/hacker-cup/2021/round-1/problems/A3
   * Points/Time :
   * +~6m
   * +28m ~ 34m :-/
   *
   * Total/ETA : 21/100 pts
   * ETA ~ 1h
   *
   * Status :
   * [upsolve]
...
void solve() {
  ...
  char firstState=UNDEF;
  char lastState=UNDEF;
//   for (auto & ch: U) {
  REP(pos,SZ(U)) {
    char ch=U[pos];
    MSG(ch);
    ...
    if (ch=='.') {
      ...
      MSG(curLen);
      curG*=2; curG%=MOD;
      curG+=( curLen*(sumPrefixes+sumSuffixes) ) % MOD;   // (*)
      MSG(curG);

      // updates
      sumPrefixes = (2*sumPrefixes + curLen * curF) % MOD;
      sumSuffixes = (2*sumSuffixes + curLen * curF) % MOD;
      curF*=2;
      if (firstState!=lastState) { // extra handswitch in-between the 2 replicates
        assert(firstState!=UNDEF);
        assert(lastState!=UNDEF);
//         sumPrefixes += curLen%MOD;  // <- bug here
        sumPrefixes += (curLen-firstPos)%MOD;
//         sumSuffixes += curLen%MOD;  // <- bug here
        sumSuffixes += (lastPos+1)%MOD;
        curF++;

        curG+=( (lastPos+1) * (curLen-firstPos) ) % MOD;
        MSG(curG);
      }
      ...
      if (ch!='F') {
        ...
        if (lastState!=UNDEF && ch!=lastState) {
          curF++;
          assert(lastPos!=CLEAN);
          sumSuffixes += (lastPos+1) % MOD;
        }
