/* ========================================
   ...
   * Last Modified : Fri 27 Aug 2021 10:20:50 PM CEST
...
void solve() {
  ...
  MSG(nVowels); MSG(nCons); LINESEP1;
  ...
  MSG(bestVow); MSG(bestCons); LINESEP1;

  // all to bestVow
  ll resultVow = 0;
  for (auto & ch: S) {
    if (!std::count(ALL(vowels), ch)) {  // cons
      resultVow+=1;
    } else if (ch!=bestVow) {  // vow
      resultVow+=2;
    }
  }
  ...
  ll resultCons = 0;
  for (auto & ch: S) {
    if (std::count(ALL(vowels), ch)) {  // vow
      resultCons+=1;
    } else if (ch!=bestCons) {  // cons
      resultCons+=2;
    }
  }

  cout << min(resultVow, resultCons) << endl;
}
