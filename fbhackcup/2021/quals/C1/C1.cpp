/* ========================================
   ...
   * Last Modified : Sat 28 Aug 2021 04:09:41 PM CEST
...
void solve() {
  ...
  function<void(ll, ll)> dfs = [&](ll at, ll parent) {
    predecMasks[at] = predecMasks[parent] | (1LL<<at);
    gains[at] = gains[parent] + C[at];
  ...
  auto commonAncestors = [&](ll v1, ll v2) {
    return predecMasks[v1]&predecMasks[v2];
  };
  auto tunnelable = [&](ll v1, ll v2) {
    return commonAncestors(v1, v2)==1;
  };
  ...
  REP(v1,N) {
    FOR(v2,v1,N-1) {
      if (v1==v2) {
        umax(result, gains[v1]);
      } else if (tunnelable(v1,v2)) {
        umax(result, gains[v1]+gains[v2]-C[0]);  // C[0] double-counted
      }
    }
  }
  cout << result << endl;
