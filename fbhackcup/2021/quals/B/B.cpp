/* ========================================
   ...
   * Last Modified : Sat 28 Aug 2021 12:49:03 AM CEST
...
void solve() {
  ...
  ll maxRowX = *max_element(ALL(rowCntX));
  ll maxColX = *max_element(ALL(colCntX));
  ll maxX = max(maxRowX, maxColX);
  if (maxX<0) {
  ...
  ll minAdd=N-maxX;
  ll nSol = std::count(ALL(rowCntX), maxX) + std::count(ALL(colCntX), maxX);

  if (minAdd==1) {
    REP(r,N) {
      REP(c,N) {
        if (C[r][c]=='.' && rowCntX[r]==maxX && colCntX[c]==maxX) {
          nSol--;  // b/c double-counted
        }
      }
    }
  }

  cout << minAdd << " " << nSol << endl;
}
