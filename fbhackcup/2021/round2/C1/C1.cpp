...
void solve() {
  ...
  ll result = std::count(ALL(G[K]), 'X');
  // try upwards
  vector<string> GG=G;
  FOR(shift,1,R-1) {
    FOR(row,1,R-1) {
      REP(col,C) {
        if (GG[row-1][col]=='.' && GG[row][col]=='X') {
          GG[row-1][col]='X';
          GG[row][col]='.';
        }
      }
    }
    umin(result, shift + (ll)std::count(ALL(GG[K]), 'X'));
  }
