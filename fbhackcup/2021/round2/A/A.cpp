/* ========================================
   ...
   * Last Modified : Sat 25 Sep 2021 09:59:37 PM CEST
...
void solve() {
  ...
  unordered_map<ll, ll> prevWear;
  unordered_map<ll, ll> wear;
  unordered_map<ll, ll> capacUnder;
  for (auto & Si: S) {
    prevWear[Si]++;
    capacUnder[Si]++;
  }
  MSG(capacUnder); MSG(prevWear); LINESEP1;
  ...
  REP(i,N) {
    ...
    MSG(prevWear); MSG(wear);
    MSG(capacUnder);

    ll nFixed=0;
    ll nAvail=0;
    unordered_map<ll, ll> fixedWear;
    for (auto [style, cnt]: wear) {
      if (CONTAINS(prevWear, style)) {
        ll delta=min(wear[style], prevWear[style]);
        nFixed+=delta;
        fixedWear[style]+=delta;
      }

      // free up models w/ unassigned underclothes as available
      if (prevWear[style]>wear[style] && capacUnder[style]>0) {
        ll delta=min(prevWear[style]-wear[style], capacUnder[style]);
        nAvail+=delta;

        capacUnder[style]-=delta;
//         if (capacUnder[style]<=0) {
//           capacUnder.erase(style);
//         }
      }
    }
    MSG(capacUnder);

    for (auto [style, cnt]: capacUnder) {
      if (CONTAINS(fixedWear, style)) {
        umin(capacUnder[style], fixedWear[style]);
      } else {
        capacUnder[style]=0;
//         capacUnder.erase(style);
      }
    ...
    MSG(fixedWear);
    MSG(capacUnder);
    result+=max(0LL,M-nFixed-nAvail);
