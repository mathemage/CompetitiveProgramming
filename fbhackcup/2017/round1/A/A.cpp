/* ========================================
   ...
   * Last Modified : Thu 08 Sep 2022 08:19:30 PM CEST
...
void solve() {
  ll N,M; cin >> N >> M;
  vector<vector<ll>> C(N, vector<ll>(M)); cin >> C;
  ...
  while (--countdown>=0) {
    ...
    ll mn=INF_LL, mx=-INF_LL;
    ll rMin=CLEAN, rMax=CLEAN;
    ll diff=-INF_LL;
    
    ll bestDiff=0;
    ll bestrMin=CLEAN, bestrMax=CLEAN;
    REP(r,N) {
//       MSG(r);
      if (bounded(cur[r]+1,M) && umin(mn, C[r][cur[r]+1])) {
        rMin=r;
      }
      if (bounded(cur[r],M) && umax(mx, C[r][cur[r]])) {
        rMax=r;
      }
//       MSG(rMin); MSG(rMax);

      if (mx!=-INF_LL && mn!=INF_LL && umax(diff, mx-mn)) {
        bestrMin=rMin;
        bestrMax=rMax;
      }
//       MSG(diff);
...
//       LINESEP1;
    ...
    if (umax(bestDiff, diff)) {
      result-=bestDiff;
      if (bounded(bestrMin,M)) {
        cur[bestrMin]++;
      }
      if (bounded(bestrMax,M)) {
        cur[bestrMax]--;
      }
