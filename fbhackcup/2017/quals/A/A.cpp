/* ========================================
   ...
   * Last Modified : Mon 22 Aug 2022 12:49:34 AM CEST
   * Created By : Karel Ha <mathemage@gmail.com>
   * URL : https://www.facebook.com/codingcompetitions/hacker-cup/2017/qualification-round/problems/A
   * Points/Time :
   * ~ 30 m
   * + 10 m = 40 m :-/
   * + 10 m ~ 50 m :-/ :-/
...
void solve() {
  ...
  bool withinDist = (dist <= 50.0);
  MSG(dist);

//   <0,50> dot-product <X,Y>
//   ld dotProd = 50.0*Y;
//   ld angle = acos(dotProd / dist / 50.0) / (2*PI);
  ld dotProd = Y;
  ld angle = acos(dotProd / dist ) * 180.0 / PI;
  MSG(dotProd);
  MSG(angle);
  ...
  ld threshold = (P/100.0 * 360.0);
  bool withinAngle = angle <= threshold;
  MSG(threshold);

  bool result = withinDist && withinAngle;
  cout << ((result)?"black":"white") << endl;
}
