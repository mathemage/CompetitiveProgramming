/* ========================================
   ...
   * Last Modified : Sat 27 Aug 2022 11:37:26 PM CEST
   * Created By : Karel Ha <mathemage@gmail.com>
   * URL : https://www.facebook.com/codingcompetitions/hacker-cup/2022/qualification-round/problems/B2
   * Points/Time :
   * Total/ETA : 17 pts
   * Status :
   *
...
ostream& operator<<(ostream& os, const vector<string> & vec) { for (const auto & s: vec) os << s << endl; return os; }
...
void solve() {
  ll R,C; cin >> R >> C;
  vector<string> grid(R); cin >> grid;
  MSG_VEC_VEC(grid);

  vector<vector<bool>> doomed(R, vector<bool>(C));

  auto countAdj = [&](ll r, ll c) {
    ll nNeigh=0;
    for (auto & dxy: DXY4) {
      ll rr=r+dxy.F;
      ll cc=c+dxy.S;
      if (bounded(rr,R) && bounded(cc,C) && grid[rr][cc]!='#' && !doomed[rr][cc]) {
//         MSG(rr); MSG(cc); MSG(grid[rr][cc]);
        nNeigh++;
      }
    }
    return nNeigh;
  ...
  while (!rs.empty()) {
    ...
    for (auto & dxy: DXY4) {
      ll rr=r+dxy.F; ll cc=c+dxy.S;
      if (bounded(rr,R) && bounded(cc,C) && grid[rr][cc]!='#' && !doomed[rr][cc]) {
        ll nNeighOfNeigh=countAdj(rr,cc);
        if (nNeighOfNeigh<2) {
          doomed[rr][cc]=true;
          rs.push(rr); cs.push(cc);
        }
      }
    }
  }
  ...
  bool doomedTree=false;
  REP(r,R) {
    REP(c,C) {
      if (grid[r][c]=='^' && doomed[r][c]) {
        doomedTree=true;
        break;
      }
