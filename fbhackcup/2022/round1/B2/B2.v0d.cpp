/* ========================================
   ...
   * Last Modified : Sun 11 Sep 2022 06:37:48 PM CEST
...
void solve() {
  ...
  map<ll,ll> A2freq; map<ll,ll> B2freq;
  REP(i,N) {
    cin >> A[i] >> B[i];
    A2freq[A[i]]++;
    B2freq[B[i]]++;
  ...
  MSG(A); MSG(B);
  LINESEP1;
  MSG(A2freq); MSG(B2freq);
  LINESEP2;

  /**********************/
  /* PREPROCESS - START */
  vector<ll> Avalues; vector<ll> Bvalues;
  for (auto & [coord, _]: A2freq) {
    Avalues.PB(coord);
  }
  MSG(Avalues);
  for (auto & [coord, _]: B2freq) {
    Bvalues.PB(coord);
  }
  MSG(Bvalues);
  LINESEP1;

  vector<unordered_map<ll, ll>> ABcoord2lCnt(2);
  vector<unordered_map<ll, ll>> ABcoord2rCnt(2);

  REP(iDim,2) {
    MSG(iDim);
    auto & dim = (iDim==0) ? Avalues : Bvalues;
    auto & freq = (iDim==0) ? A2freq : B2freq;

    auto & lCnt = ABcoord2lCnt[iDim];
    for (ll i = 0; i < SZ(dim); i += 1) {
      ll coord=dim[i];
      lCnt[coord] = (i==0 ? 0 : lCnt[dim[i-1]]) + freq[coord];
    }

    auto & rCnt = ABcoord2rCnt[iDim];
    FORD(i,SZ(dim)-1,0) {
      ll coord=dim[i];
      rCnt[coord] = (i+1>=SZ(dim) ? 0 : rCnt[dim[i+1]]) + freq[coord];
    }
  ...
  auto sumInDimension = [&](ll qCoord, const map<ll,ll> & coord2freq) {
#warning TODO implement
  ...
  REP(i,Q) {
    ...
    result += sumInDimension(Xi, A2freq);
    result %= MOD;

    result += sumInDimension(Yi, B2freq);
    result %= MOD;
  }
