/* ========================================
   ...
   * Last Modified : Wed 14 Sep 2022 07:47:22 PM CEST
   * Created By : Karel Ha <mathemage@gmail.com>
   * URL : https://www.facebook.com/codingcompetitions/hacker-cup/2022/round-1/problems/B2
   * Points/Time :
   * Total/ETA : 18 pts
   * Status :
   * AC!!
...
void solve() {
  ...
  REP(_,N) {
    cin >> Ai;
    sum1x+=Ai; sum1x%=MOD;
    sum2x+=(Ai*Ai)%MOD; sum2x%=MOD;

    cin >> Bi;
    sum1y+=Bi; sum1y%=MOD;
    sum2y+=(Bi*Bi)%MOD; sum2y%=MOD;
  }
  ...
  REP(_,Q) {
    cin >> Xi;
    result += (N*((Xi*Xi)%MOD))%MOD; result %= MOD;
    result += (sum2x)%MOD; result %= MOD;
    result += (MOD-(2*sum1x*Xi)%MOD)%MOD; result %= MOD;

    cin >> Yi;
    result += (N*((Yi*Yi)%MOD))%MOD; result %= MOD;
    result += (sum2y)%MOD; result %= MOD;
    result += (MOD-2*(sum1y*Yi)%MOD)%MOD; result %= MOD;
  }
