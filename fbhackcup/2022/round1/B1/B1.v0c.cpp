/* ========================================
   ...
   * Last Modified : Sun 11 Sep 2022 04:43:54 PM CEST
...
void solve() {
  ...
  map<ll,ll> memoX; map<ll,ll> memoY;

  auto sumInDimension = [&](ll qCoord, const map<ll,ll> & coord2freq, map<ll,ll> & memo) {
  ...
  REP(i,Q) {
    ...
    result += sumInDimension(X[i], A2freq, memoX);
    result %= MOD;

    result += sumInDimension(Y[i], B2freq, memoY);
    result %= MOD;
  }
