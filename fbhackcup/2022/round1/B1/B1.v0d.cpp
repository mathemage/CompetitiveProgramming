/* ========================================
   ...
   * Last Modified : Sun 11 Sep 2022 04:52:18 PM CEST
...
void solve() {
  ...
  unordered_map<ll,ll> memoX; unordered_map<ll,ll> memoY;

  auto sumInDimension = [&](ll qCoord, const map<ll,ll> & coord2freq, unordered_map<ll,ll> & memo) {
    if (!CONTAINS(memo, qCoord)) {
      ll ans=0;
      for (auto & [coord, freq]: coord2freq) {
        ans+=freq*pow(coord-qCoord, 2);
        ans%=MOD;
      }
      memo[qCoord]=ans;
    }
...
//   MSG("here");
  ...
  REP(i,Q) {
    ...
    result += sumInDimension(Xi, A2freq, memoX);
    result %= MOD;

    result += sumInDimension(Yi, B2freq, memoY);
    result %= MOD;
  }
