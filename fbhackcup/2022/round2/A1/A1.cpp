/* ========================================
   ...
   * Last Modified : Sat 24 Sep 2022 08:10:40 PM CEST
...
void solve() {
  ...
  // preprocess
  vector<vector<ll>> prefHisto;
  vector<ll> curHisto(26,0);
  REP(i,N) {
    curHisto[S[i]-'a']++;
    prefHisto.PB(curHisto);
  }
  MSG_VEC_VEC(prefHisto);

  auto getHisto = [&](ll L, ll R) {
    MSG(L); MSG(R);
    assert(L<=R);

    vector<ll> histo = prefHisto[R];
    if (L-1>=0) {
      REP(ci,26) {
        histo[ci]-=prefHisto[L-1][ci];
      }
    }
...
//     LINESEP1;
  ...
  REP(_,Q) {
    ...
    for (auto & pivot: {mid-1, mid}) {
      MSG(pivot);
      if (Li<=pivot && pivot<=Ri) {
        vector<ll> h1=getHisto(Li,pivot);
        MSG(h1);
        vector<ll> h2=getHisto(pivot+1,Ri);
        MSG(h2);
