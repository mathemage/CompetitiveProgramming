/* ========================================
   ...
   * Last Modified : Sat 24 Sep 2022 10:01:50 PM CEST
...
struct FT {
  ...
  int lower_bound(ll sum) {// min pos st sum of [0, pos] >= sum Returns n if no sum is >= sum, or -1 if empty sum is.
    if (sum <= 0) return -1;
    int pos = 0;
