/* ========================================
   ...
   * Last Modified : Fri 10 Sep 2021 12:15:41 AM CEST
...
void solve() {
  ll N,M; cin >> N >> M;
  vector<vector<ll>> edges(M, vector<ll>(3)); cin >> edges;
  ...
  for (auto & e: edges) {
    ...
    dist[j][i] = dist[i][j] = e[2];
    orig[j][i] = orig[i][j] = true;
  }

  REP(k,N) {
    REP(i,N) {
      REP(j,N) {
        if (umin(dist[i][j], dist[i][k]+dist[k][j]) && orig[i][j]) {
