/* ========================================
   ...
   * Last Modified : Fri 27 Aug 2021 09:12:05 PM CEST
...
//   ld dp[mx][mx][mx]{};  // TODO????
  ...
  vector<vector<vector<ld>>> dp(305,
    vector<vector<ld>>(305,
      vector<ld>(305, 0.0)
    )
  );
  ...
  REP(n3,N+1) {
    REP(n2,N+1) {
      REP(n1,N+1) {
        MSG(n1); MSG(n2); MSG(n3);

        if (n1+n2+n3!=0) {
          if (n1-1>=0) { dp[n1][n2][n3] += ld(n1) * dp[n1-1][n2][n3]; }
          if (n2-1>=0) { dp[n1][n2][n3] += ld(n2) * dp[n1+1][n2-1][n3]; }
          if (n3-1>=0) { dp[n1][n2][n3] += ld(n3) * dp[n1][n2+1][n3-1]; }
          MSG(dp[n1][n2][n3]);
//           dp[n1][n2][n3]/=ld(N);
          dp[n1][n2][n3]/=ld(n1+n2+n3);
          MSG(dp[n1][n2][n3]);

          dp[n1][n2][n3]+=ld(N)/ld(n1+n2+n3);
          MSG(ld(N)/ld(n1+n2+n3));
          MSG(dp[n1][n2][n3]);
        }
