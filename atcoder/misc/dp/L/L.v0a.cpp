/* ========================================
   ...
   * Last Modified : Mon 04 Oct 2021 11:23:01 PM CEST
...
void solve() {
  ...
  ll dim=N+5;
  vector<vector<ll>> dpMn(dim, vector<ll>(dim, UNDEF));
  vector<vector<ll>> dpMx(dim, vector<ll>(dim, UNDEF));
  REP(i,N) {
    dpMx[i][i]=dpMn[i][i]=a[i];
  }

  FOR(len,2,N) {
    for (ll i = 0; i+len-1 < N; i += 1) {
      ll j=i+len-1;
      dpMn[i][j] = min(dpMx[i+1][j]-a[i], dpMx[i][j-1]-a[j]);
      dpMx[i][j] = max(dpMn[i+1][j]+a[i], dpMn[i][j-1]+a[j]);
    }
  }

  ll result = dpMx[0][N-1];
  cout << result << endl;
}
