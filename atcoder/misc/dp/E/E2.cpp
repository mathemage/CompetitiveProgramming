/* ========================================
   ...
   * Last Modified : Wed 14 Jul 2021 04:35:13 PM CEST
...
void solve() {
  ...
  MSG(w); MSG(v); LINESEP1;

  vector<long long> dp(totalV+10, INF_LL/2);
  dp[0]=0;
  MSG(totalV); MSG(SZ(dp)); LINESEP1;

  REP(i,N) {
    FORD(sumV,totalV,v[i]) {
      MSG(dp[sumV]); MSG(dp[sumV-v[i]]+w[i]);

      umin(dp[sumV], dp[sumV-v[i]]+w[i]);
      MSG(dp[sumV]); LINESEP1;
    }
  }
