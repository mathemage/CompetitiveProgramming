/* ========================================
   ...
   * Last Modified : Sat 27 Nov 2021 01:14:37 PM CET
...
void solve() {
  ...
  bool result = 1;
//   for (ll i = min(SZ(A), SZ(B))-2; i >= 0; i += 1) {
  for (ll i = 0; i < min(SZ(A),SZ(B)); i += 1) {
    if (A[i]-'0' + B[i]-'0' > 9) {
      MSG(i); MSG(A[i]); MSG(B[i]);
      MSG(A[i]-'0' + B[i]-'0');
