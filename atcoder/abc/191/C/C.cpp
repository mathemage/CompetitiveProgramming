/* ========================================
   ...
   * Last Modified : Sat 29 May 2021 08:52:42 PM CEST
   * Created By : Karel Ha <mathemage@gmail.com>
   * URL : https://atcoder.jp/contests/abc191/submissions/me
   * Points/Time : 28m
   * Total/ETA : 300
   * Status : AC :-O uff!
   ==========================================*/
...
void solve() {
  ll H,W; cin >> H >> W;
  vector<string> S(H, string(W, '_')); cin >> S;
  MSG(S);
  ...
  FOR(c,1,W-2) {
    FOR(r,1,H-2) {
      if (S[r][c]=='#' && S[r+1][c]=='.') {
        ...
        result+=!(S[r][c-1]=='#' && S[r+1][c-1]=='.');
      }
      if (S[r-1][c]=='.' && S[r][c]=='#') {
        MSG(r) MSG(c); LINESEP1;
        result+=!(S[r-1][c-1]=='.' && S[r][c-1]=='#');
      }
    }
  }
  ...
  FOR(r,1,H-2) {
    FOR(c,1,W-2) {
      if (S[r][c]=='#' && S[r][c+1]=='.') {
        MSG(r) MSG(c); LINESEP1;
        result+=!(S[r-1][c]=='#' && S[r-1][c+1]=='.');
      }
      if (S[r][c-1]=='.' && S[r][c]=='#') {
        MSG(r) MSG(c); LINESEP1;
        result+=!(S[r-1][c-1]=='.' && S[r-1][c]=='#');
      }
    }
  }
