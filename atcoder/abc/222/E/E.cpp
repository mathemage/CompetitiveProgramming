/* ========================================
   ...
   * Last Modified : Sat 09 Oct 2021 03:41:44 PM CEST
...
void solve() {
  ...
  unordered_map<ll, unordered_map<ll, ll>> edge2arity;

  vector<ll> path;
  vector<bool> vis(N);
  function<void(ll, ll)> dfs = [&](ll at, ll end) {
    if (at==end) {
      for (ll i = 0; i+1 < SZ(path); i += 1) {
        ll u=min(path[i], path[i+1]);
        ll v=max(path[i], path[i+1]);
        edge2arity[u][v]++;
      }

#warning TODO prematurely terminate DFS
    }
