/* ========================================
   ...
   * Last Modified : Sat 09 Oct 2021 02:31:37 PM CEST
...
inline bool winsOver(char c1, char c2) {
  return (c1=='G'&&c2=='C') ||
         (c1=='C'&&c2=='P') ||
         (c1=='P'&&c2=='G');
}
...
void solve() {
  ...
  ll nPlayers = 2*N;
  vector<ll> rank2ids(nPlayers);
  std::iota(ALL(rank2ids), 0);
  vector<ll> totalWins(nPlayers);

  REP(round,M) {
    FOR(k,1,N) {
      ll pl1 = rank2ids[2*k-1-1];
      ll pl2 = rank2ids[2*k-1];
      MSG(pl1); MSG(pl2);

      totalWins[pl1]+=winsOver(A[pl1][round], A[pl2][round]);
      totalWins[pl2]+=winsOver(A[pl2][round], A[pl1][round]);
    }

    vector<ll> ranks(nPlayers);
    std::iota(ALL(ranks), 0);
    std::stable_sort(ALL(ranks), [&](const auto & a, const auto & b) {
        return totalWins[a]>totalWins[b]
          || (totalWins[a]==totalWins[b] && a<b);
        }
      );
