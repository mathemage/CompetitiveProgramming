/* ========================================
   ...
   * Last Modified : Sun 30 May 2021 03:34:58 PM CEST
...
typedef tree<ll,null_type,greater<ll>,rb_tree_tag,tree_order_statistics_node_update> indexed_set;

void solve() {
  ll N,K; cin >> N >> K;
  vector<vector<long long>> A(N, vector<long long>(N)); cin >> A;
...
//   ll iMedian=(K*K)/2+1;
  ll iMedian=(K*K)/2;
  ll result = *st.find_by_order(iMedian);
  MSG(iMedian);

  for (ll r = 0; r+K-1 < N; r += 1) {
    if (r-1>=0) {
      REP(dc,K) {
        st.erase(A[r-1][0+dc]);      // remove hor side
        st.insert(A[r+K-1][0+dc]); // add new hor side
      }
      MINUPDATE(result, *st.find_by_order(iMedian));
    }

    indexed_set st2=st;
    for (ll c = 1; c+K-1 < N; c += 1) {
      REP(dr,K) {
        st2.erase(A[r+dr][c-1]);      // remove vert side
        st2.insert(A[r+dr][c+K-1]); // add new vert side
      }
      
      // update result with median
      MINUPDATE(result, *st2.find_by_order(iMedian));
    }
  }
