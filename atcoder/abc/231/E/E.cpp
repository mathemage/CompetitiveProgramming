/* ========================================
   ...
   * Last Modified : Sat 11 Dec 2021 02:46:18 PM CET
   ...
   * Status :
   * ACx7, WAx30
   * [unsubmitted in-contest] - ACx9, WAx7, TLEx22
   *
   ==========================================*/
...
void solve() {
  ...
  function<ll(ll)> optCnt = [&](ll val) {
    MSG(val);
    auto it = std::upper_bound(ALL(A), val)-1;
    ll maxCoin = *it;
    ll cntCoin = val/maxCoin;
    return val<=0 ? 0 : cntCoin+optCnt(val-cntCoin*maxCoin);
  };

//   ll result = INF_LL;
  ll result = optCnt(X);
  REP(d,1e7) {
    ll curRes = optCnt(X+d)+optCnt(d);
    umin(result, curRes);
  }
//   for (auto & Ai: A) {
//     ll curRes = (X+Ai-1)/Ai;
//     assert(curRes * Ai >= X);
//     ll delta = curRes * Ai - X;
// 
//     curRes += optCnt(delta);
//     umin(result, curRes);
//   }
