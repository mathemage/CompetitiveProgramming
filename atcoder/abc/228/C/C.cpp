/* ========================================
   ...
   * Last Modified : Sat 20 Nov 2021 01:43:26 PM CET
...
void solve() {
  ...
  vector<ll> sortedScores=scores;

  std::sort(ALL(sortedScores));
  MSG(sortedScores); LINESEP1;

  for (auto & sc: scores) {
//     ll rank = N-1+std::upper_bound(ALL(sortedScores), sc)-sortedScores.begin();
    ll rank = 1+sortedScores.end()-std::upper_bound(ALL(sortedScores), sc+300);
    MSG(sc); MSG(rank);
