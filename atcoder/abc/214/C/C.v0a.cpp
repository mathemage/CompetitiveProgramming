...
/* ========================================
   ...
   * Last Modified : So 14. srpna 2021, 14:48:49
...
template<typename T1, typename T2> 
ostream& operator<<(ostream& os, const multimap<T1, T2>& vec) { for (const auto & x: vec) os << x.F << ":" << x.S << " | "; return os; }
...
void solve() {
  ...
  multimap<ll,ll> time2id;

  vector<ll> result(N,INF);
  REP(i,N) {
    time2id.insert({T[i], i});
  }
  MSG(time2id); LINESEP1;

  ll mxT=*max_element(ALL(T));

  while (!time2id.empty()) {
    auto it=time2id.begin();
    ll time=it->F;
    ll id=it->S;
    time2id.erase(it);
    MSG(time); MSG(id);

    if (umin(result[id], time)) {
      ll ne=(id+1)%N;
      if (result[ne]>time+S[id]) {
//         time2id.erase({result[ne],ne});
        result[ne]=time+S[id];
        time2id.insert({result[ne],ne});
      }
    }
    MSG(time2id);
    LINESEP1;
