/* ========================================
   ...
   * Last Modified : Sat 15 May 2021 03:22:05 PM CEST
...
void solve() {
  ll H,W; cin >> H >> W;
  vector<string> A(H, string(W,'_')); cin >> A;
  MSG_VEC_VEC(A);

  vector<vector<ll>> grid(H, vector<ll>(W));
  REP(r,H) {
    REP(c,W) {
      grid[r][c]=(A[r][c]=='+'? 1LL:-1LL);
    }
  }
  MSG_VEC_VEC(grid);

  vector<vector<ll>> adv(H, vector<ll>(W));
  ll r,c;
  FORD(pos,H*W-1,0) {
    r=pos/W; c=pos%W;
    ll playerFactor=((r+c)&1 ? -1: 1);

    if ((r+c)&1) { // Aoki
      adv[r][c]=INF_LL;
      if (bounded(r+1,H)) {
        MINUPDATE(adv[r][c], playerFactor*grid[r+1][c] + adv[r+1][c]);
      }
      if (bounded(c+1,W)) {
        MINUPDATE(adv[r][c], playerFactor*grid[r][c+1] + adv[r][c+1]);
      }
    } else { // Takahashi
      adv[r][c]=-INF_LL;
      if (bounded(r+1,H)) {
        MAXUPDATE(adv[r][c], playerFactor*grid[r+1][c] + adv[r+1][c]);
      }
      if (bounded(c+1,W)) {
        MAXUPDATE(adv[r][c], playerFactor*grid[r][c+1] + adv[r][c+1]);
      }
    }

    if (adv[r][c]==INF_LL || adv[r][c]==-INF_LL) {
      adv[r][c]=0;
    }
  }
  MSG_VEC_VEC(adv);
