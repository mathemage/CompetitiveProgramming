/* ========================================
   ...
   * Last Modified : Sun 08 Aug 2021 02:27:35 PM CEST
...
void solve() {
  ...
  vector<ll> C(N);
  ll r=1;
  for (auto & rowPos: cntR) {
    for (auto & pos: rowPos.S) {
      C[pos]=r;
    }
    r++;
  }
  ...
  vector<ll> D(N);
  ll c=1;
  for (auto & colPos: cntC) {
    for (auto & pos: colPos.S) {
      D[pos]=c;
    }
    c++;
  }
