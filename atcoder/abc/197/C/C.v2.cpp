/* ========================================
   ...
   * Last Modified : Sat 27 Mar 2021 03:16:08 PM CET
...
void solve() {
  ...
  for (long long bitmask = 0LL; bitmask < 1<<(N-1); bitmask += 1) {
    ...
    FOR(i,1,N-1) {
      ...
      MSG(bitmask); MSG(i-1);
      LINESEP1;
      MSG(bitmask>>(i-1)); MSG( (bitmask>>(i-1))&1 );
      LINESEP1;

      if ( (bitmask>>(i-1))&1 ) { // <- NECESSARY !!!
//       if ( bitmask>>(i-1) ) { // split
        xorSum^=orSum;
        orSum=a[i];
      } else {                // join with prev
        orSum|=a[i];
    ...
    xorSum^=orSum;
//     MINUPDATE(result, xorSum & ((1LL<<35)-1LL));   // <- `& ((1LL<<35)-1LL)` is unnecessary !!!
