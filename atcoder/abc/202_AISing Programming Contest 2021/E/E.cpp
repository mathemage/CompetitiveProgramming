/* ========================================
   ...
   * Last Modified : Sat 22 May 2021 03:42:06 PM CEST
...
void solve() {
  ll N; cin >> N;
  vector<long long> P(N-1); cin >> P;

  unordered_map<ll, vector<long long>> children;
  FOR(i,2,N) {
    children[P[i-2]].PB(i);
  }
  MSG(children); LINESEP1;

  // BFS
  queue<ll> q;
  vector<long long> dist(1+N, UNDEF);
  q.push(1); dist[1]=0;
  ...
  while (!q.empty()) {
    ...
    for (auto & child: children[at]) {
      dist[child]=dist[at]+1;
      q.push(child);
    }
  }
  MSG(dist); LINESEP1;

  vector<vector<long long>> depths(N);
  FOR(at,1,N) {
    depths[dist[at]].PB(at);
  }
