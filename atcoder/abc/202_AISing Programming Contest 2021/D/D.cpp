/* ========================================
   ...
   * Last Modified : Sat 22 May 2021 03:09:27 PM CEST
...
void solve() {
  ll A,B,K; cin >> A >> B >> K;
  ll AA=A, BB=B;
  MSG(A); MSG(B); MSG(K); LINESEP1;

  ll nRows=1+A;
  ll nCols=1+B;
  vector<vector<long long>> dp(nRows, vector<long long>(nCols, 1LL));
  FOR(r,1,nRows-1) {
    FOR(c,1,nCols-1) {
...
//   while (K>0) {
  ...
  while (B>0) { result+='b'; B--; }

  assert(SZ(result)==AA+BB);
  assert(count(ALL(result), 'a')==AA);
  assert(count(ALL(result), 'b')==BB);
