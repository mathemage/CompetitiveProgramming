...
/* ========================================
   ...
   * Last Modified : So 8. května 2021, 15:14:37
...
void solve() {
  ...
  vector<vector<vector<int>>> seqOfSeqs(200);
  REP(i,N) {
    vector<pair<int, vector<int>>> newSeqs;
    newSeqs.PB({A[i], {i+1}});

    REP(modSum,200) {
      for (auto & seq: seqOfSeqs[modSum]) {
        vector<int> newSeq=seq; newSeq.PB(i+1);
        newSeqs.PB({ (modSum+A[i])%200, newSeq });
      }
    }
    MSG(newSeqs); LINESEP1;

    for (auto & modSeq: newSeqs) {
      int modSum=modSeq.F;
      vector<int> seq=modSeq.S;

      seqOfSeqs[modSum].PB(seq);
      if (SZ(seqOfSeqs[modSum]) >= 2) {
        result[0]=seqOfSeqs[modSum][0];
        result[1]=seqOfSeqs[modSum][1];
        check=true;
        break;
      }
    }
