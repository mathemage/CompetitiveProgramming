/* ========================================
   ...
   * Last Modified : Fri 23 Jul 2021 12:23:30 AM CEST
...
void solve() {
  ...
  REP(_,2) {
    ...
    REP(r,H) {
      REP(c,W) {
        ll minOther=INF;
        if (r) { umin(minOther, dp[r-1][c]); }
        if (c) { umin(minOther, dp[r][c-1]); }
        dp[r][c]=min(minOther, A[r][c]-C*(r+c));

        umin(result, A[r][c]+C*(r+c)+minOther);
      }
    }
