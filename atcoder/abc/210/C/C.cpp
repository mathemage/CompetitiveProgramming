...
/* ========================================
   ...
   * Last Modified : So 17. července 2021, 14:46:27
...
void solve() {
  ll N,K; cin >> N >> K;
  vector<long long> c(N); cin >> c;

  ll curCnt=0LL;
  unordered_map<ll,ll> col2cnt;
  REP(i,K) {
    if (++col2cnt[c[i]]==1) {
      curCnt++;
    }
  ...
  ll result = curCnt;
  MSG(result); LINESEP1;
  for (ll i = 1; i+K-1 < N; i += 1) {
    ll colHead=c[i-1];
    ll colTail=c[i+K-1];
    MSG(i); MSG(i+K-1); 
    MSG(colHead); MSG(colTail); 

    if (++col2cnt[colTail]==1) {
      curCnt++;
    }
    if (--col2cnt[colHead]==0) {
      col2cnt.erase(colHead); // TODO
      curCnt--;
    }
