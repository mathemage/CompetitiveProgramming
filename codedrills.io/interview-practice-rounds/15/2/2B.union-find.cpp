...
#define MATHEMAGE_DEBUG // TODO comment out

class ValidTree {
public:
	bool isValidTree(std::vector<std::vector<int>> edges, int nodes) {
    int n=nodes;
    vector<int> link(n);
    std::iota(ALL(link), 0);

    function<int(int)> find = [&](int at) -> int {
      return link[at]==at ? at : link[at]=find(link[at]);
    };
