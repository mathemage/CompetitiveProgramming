/* ========================================
   ...
   * Last Modified : Thu 31 Mar 2022 11:47:45 PM CEST
   * Created By : Karel Ha <mathemage@gmail.com>
   * URL : https://codingcompetitions.withgoogle.com/codejam/round/00000000000000cb/00000000000079cb#problem
   * Points/Time :
   * = 6m
   *
   * Total/ETA : 8+15 pts
...
void solve() {
  ...
  MSG(halves[0]); MSG(halves[1]);
  LINESEP2;

  std::sort(ALL(halves[0]));
  std::sort(ALL(halves[1]));
  MSG(halves[0]); MSG(halves[1]);
  LINESEP2;

  string result = "OK";
  REP(k,N/2) {
    if (k<SZ(halves[1]) && halves[0][k]>halves[1][k]) {
      result = to_string(2*k);
      break;
    }

    if (k+1<SZ(halves[0]) && halves[1][k]>halves[0][k+1]) {
      result = to_string(2*k+1);
      break;
    }
  }
