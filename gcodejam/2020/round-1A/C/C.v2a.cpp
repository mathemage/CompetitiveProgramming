...
/* ========================================
   ...
   * Last Modified : Čt 8. dubna 2021, 19:35:58
...
void solve() {
  ...
  vector<vector<vector<int>>> neigh(nRows, vector<vector<int>>(nCols, vector<int>(4, UNDEF)));
  REP(i,nRows) {
    REP(j,nCols) {
      cin >> S[i][j];
      round += S[i][j];
      ...
      for (auto & due: {U,D}) {
        ...
        if (bounded(x,nRows) && bounded(y,nCols)) { neigh[i][j][due]=y; }
        else { neigh[i][j][due]=UNDEF; }
      }

      for (auto & due: {R,L}) {
        x=i+DXY4[due].F, y=j+DXY4[due].S;
        if (bounded(x,nRows) && bounded(y,nCols)) { neigh[i][j][due]=x; }
        else { neigh[i][j][due]=UNDEF; }
      }
    }
  }
  competition+=round;
  MSG_VEC_VEC(S); MSG_VEC_VEC(neigh); MSG(round); MSG(competition);
    ...
    REP(i,nRows) {
      REP(j,nCols) {
        if (S[i][j]!=UNDEF) {
          ...
          for (auto & dxy: DXY4) {
            x=i+dxy.F, y=j+dxy.S;
            while (bounded(x,nRows) && bounded(y,nCols) && S[x][y]==UNDEF) {
              x+=dxy.F, y+=dxy.S;
            }
            if (bounded(x,nRows) && bounded(y,nCols) && S[x][y]!=UNDEF) {
              nSum+=S[x][y];
              nCnt++;
            }
          }
