...
//   MSG(P);
  ...
  string result=P[0].substr(1);
  FOR(i,1,N-1) {
    LINESEP1;
    MSG(result); MSG(P[i]);

    string needle=P[i].substr(1);
    MSG(needle);

    int pos=result.size()-needle.size();
    MSG(result.size()); MSG(needle.size()); MSG(pos);

    string hay=result.substr(pos);
    MSG(hay);
