...
void solve() {
  ...
  do {
    ...
    for (auto & init: initiators) {
      ...
      ll chainCost=-INF_LL;
      while (at!=ABYSS && !vis[at]) {
        umax(chainCost, F[at-1]);
        vis[at]=1;
    ...
    umax(result, totalCost);
  } while (next_permutation(ALL(initiators)));
