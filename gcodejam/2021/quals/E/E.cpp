/* ========================================
   ...
   * Last Modified : Sat 27 Mar 2021 08:46:19 PM CET
   ...
   * + 6m = 12m
   * +13m = 25m
   * + 9m = 34m (resubmission of the 1st submission)
   *
   * Total/ETA :
   * Status :
   * AC (Test Set 1), WA (Test Set 2)
   * <- (only) 2773 participants (7%) solved 1 test set
   * - see screenshot
   * AC (Test Set 1), WA (Test Set 2)
   * WA (Test Set 1 & Test Set 2)
   * AC (Test Set 1), WA (Test Set 2)
   *
   ==========================================*/
...
inline double invSigm(double y) {
  return log(y/(1-y));
}
...
//   REP(col,10000) {
//     Q[col]=invSigm((rowSums[col]-.5)/99.5);
//   }
//   MSG(Q);
// 
//   // doesn't work, nans
...
//     REP(col,10000) {
//       if (m[row][col]) {
//         MSG(sigm(Q[col])/10000.0);
//         S[row]+=sigm(Q[col])/10000.0;
//       }
//     }
//     S[row]=invSigm(S[row]);
//     if ( abs(S[row]) > abs(S[iMaxSAbs]) ) {
//       iMaxSAbs=row;
//     }
//   }
//   MSG(S);
...
//   int result = 1+max_element(ALL(scores))-scores.begin();
//   int result = 1+iMaxSAbs;    
  cout << result << endl;
}
