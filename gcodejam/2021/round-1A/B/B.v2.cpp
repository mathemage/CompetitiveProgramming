/* ========================================
   ...
   * Last Modified : Thu 15 Apr 2021 08:53:20 PM CEST
...
void search(ll sum=0, ll prod=1, int iM=0) {
  if (iM==M) {
    if (sum==prod) { MAXUPDATE(result, sum); }
    MSG(sum); MSG(prod); MSG(result); LINESEP1; LINESEP1;
    return;
  }

  ll sum2=sum+N[iM]*P[iM]; ll prod2=prod;
  FOR(_,0,N[iM]) {
    MSG(iM); MSG(P[iM]); MSG(prod2); MSG(sum2); LINESEP1;

    // TODO optimize search
    //
//     if (prod2>sum2 || prod2==sum2) { // TODO

    if (prod2>maxVal) { // TODO
      break;
    }

    search(sum2, prod2, iM+1);
...
void solve() {
  ...
  // optimizations
//   maxVal-=P[0]; // TODO
  reverse(ALL(P)); reverse(ALL(N));
  MSG(P); MSG(N); LINESEP1;
