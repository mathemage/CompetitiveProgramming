/* ========================================
   ...
   * Last Modified : Sat 01 May 2021 11:22:14 AM CEST
...
void solve() {
  ...
  ll maxWins=0;
  FOR(ticket1,1,K) {
    if (slots[ticket1]) { continue; }

    FOR(ticket2,1,K) {
      if (slots[ticket2]) { continue; }

      ll nWins=0;
      FOR(c,1,K) {
        bool ok=true;
        for (auto & v: vals) {
          if (abs(v-c)<=abs(ticket1-c) && abs(v-c)<=abs(ticket2-c)) {
            ok=false;
            break;
          }
        }
