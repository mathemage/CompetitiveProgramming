...
ul Y;
vector<ul> roaringNums;
const ul MAX_ROAR=1234567891011121314LL;
const string MAX_ROAR_STR=to_string(MAX_ROAR);

void solve() {
  cin >> Y;
  MSG(Y); LINESEP1;

  ul result = *upper_bound(ALL(roaringNums), Y);
  MSG(result); LINESEP1;

  string doubleRoar;
//   ul low=1, high=999'999'998, midRoar;  // bug
  ul low=1, high=999'999'999, midRoar;
  while (low+1<high) {
    ul mid=low + (high-low)/2;
#ifdef MATHEMAGE_DEBUG
    assert(low<=mid); assert(mid<=high);
#endif

    doubleRoar=to_string(mid)+to_string(mid+1);
    midRoar=stoull(doubleRoar);
    MSG(low); MSG(high);
    MSG(mid); MSG(midRoar);
    LINESEP1;
  ...
  doubleRoar=to_string(high)+to_string(high+1);
  midRoar=stoull(doubleRoar);
  MSG(midRoar);
  MINUPDATE(result, midRoar);
...
//   setIO(PROBLEMNAME);
  ...
  FOR(seed,1,1e6) {
    ...
    numStr=to_string(seed)+to_string(++seed2);
    roaringNums.PB(stoull(numStr));

    while (true) {
      numStr+=to_string(++seed2);
      if (SZ(numStr)<=SZ(MAX_ROAR_STR)) {
        roaringNums.PB(stoull(numStr));
      } else {
        break;
      }
