/* ========================================
   ...
   * Last Modified : Tue 26 Apr 2022 07:42:00 PM CEST
...
void solve() {
  ...
  unordered_map<char, map<ll, ll>> due2cnt;
  ...
  MSG(due2cnt);
  MSG(votesWE); MSG(votesSN);
  LINESEP2;

  ll ansX = 0LL;
  ll mxVotes = votesWE-due2cnt[ansX]['W']-due2cnt[ansX]['E'];
  ll curVotes = mxVotes;
  for (auto & [x, w]: due2cnt['W']) {
    ll e=due2cnt['E'][x];

    curVotes-=w;
    curVotes+=e;
    if (umax(mxVotes, curVotes) && x+1<=Q) {
      ansX=x+1;
    }
  }
  cout << ansX << " ";
  ...
  ll ansY = 0LL;
  mxVotes = votesSN-due2cnt[ansY]['S']-due2cnt[ansY]['N'];
  curVotes = mxVotes;
  for (auto & [y, s]: due2cnt['S']) {
    ll n=due2cnt['N'][y];

    curVotes-=s;
    curVotes+=n;
    if (umax(mxVotes, curVotes) && y+1<=Q) {
      ansY=y+1;
    }
  }
  cout << ansY << endl;
