/* ========================================
   ...
   * File Name : B.brute.cpp
   * Creation Date : 15-04-2023
   * Last Modified : So 15. dubna 2023, 17:22:47
   * Created By : Karel Ha <mathemage@gmail.com>
   * URL : https://codingcompetitions.withgoogle.com/codejam/round/0000000000c95b94/0000000000cad086
   * Points/Time : 4+10 pts
   * Total/ETA :
   * Status :
   * S TLE - :-/
   * S MLE - :-/
...
//   ll MX = int_points + int_points - 1;
  ...
  REP(b,1LL<<N) {
    ...
    REP(i,N) {
      if (b& (1LL<<i) ) {
        ll left = max(0LL, X[i]-R);
        ll right = min(M, X[i]+R);

        bool united = false;
        for (auto & LR: intervals) {
          if (left<=LR.CL) {
            ll newL = min(left, LR.OP);
            ll newR = max(right, LR.CL);
            intervals.erase(LR);
    ...
    set<pair<ll, ll>> entire{ {0,M} };
//     if (SZ(intervals)==1 && intervals. == MP(0, M)) {
//     if (SZ(intervals)==1) {
    if (intervals == entire) {
      umin(result, count);
    }
