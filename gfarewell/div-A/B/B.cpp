/* ========================================
   ...
   * Last Modified : So 15. dubna 2023, 19:07:31
...
void solve() {
  ...
  // skip 1st
//   ll left = min(M, X[0]+R);
//   ll right = min(M, left+1);

//   if (left==right) {
//     cout << 1 << endl;
//     return;
//   }
//   MSG(result); MSG(left); MSG(right); LINESEP2;

  ll left = 0;
  ll right = 1;

  while (bounded(i,N) && bounded(left,M+1) && bounded(right,M+1) && left < right) {
    while (i<N
        && max(0LL,X[i]-R)<=left
        && right<=min(M,X[i]+R) ) {
      i++;
    ...
    i--;
    MSG(i);
    if (bounded(i,N) && max(0LL,X[i]-R)<=left && right<=min(M,X[i]+R) ) {
      result++;
      left = min(M, X[i]+R);
      right = min(M, left+1);
      i++;
