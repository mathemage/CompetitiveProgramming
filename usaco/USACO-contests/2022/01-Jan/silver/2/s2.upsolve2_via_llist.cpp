/* ========================================
   ...
   ID: mathema6
   TASK: s2.upsolve2_via_llist
   LANG: C++14

   * File Name : s2.upsolve2_via_llist.cpp
   * Creation Date : 09-02-2022
   * Last Modified : Wed 09 Feb 2022 08:10:58 PM CET
...
#define PROBLEMNAME "s2.upsolve2_via_llist"
...
void solve() {
  ...
  vector<ll> prev(N+2); std::iota(ALL(prev), -1);
  vector<ll> next(N+2); std::iota(ALL(next), 1);
  MSG(prev); MSG(next);

  vector<ll> posFromSmallest(N); std::iota(ALL(posFromSmallest), 1);
  std::stable_sort(ALL(posFromSmallest), [&](const auto & i, const auto & j) { return h[i-1] < h[j-1]; } );
  MSG(posFromSmallest); LINESEP1;

  ll result = 0LL;
  for (auto & curPos: posFromSmallest) {
    if (bounded(next[curPos], 1LL, N+1)) {
      result += next[curPos] - curPos + 1;
    }

    if (bounded(prev[curPos], 1LL, N+1)) {
      result += curPos - prev[curPos] + 1;
    }
