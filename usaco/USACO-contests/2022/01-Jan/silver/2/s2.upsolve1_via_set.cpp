/* ========================================
   ...
   ID: mathema6
   TASK: s2.upsolve1_via_set
   LANG: C++14

   * File Name : s2.upsolve1_via_set.cpp
   * Creation Date : 07-02-2022
   * Last Modified : Mon 07 Feb 2022 10:39:43 PM CET
   ...
   * Status :
   * [editorial]
   * 11/11 ACs (passed) <- flash-upsolved! ^_^
   *
   ==========================================*/

#define PROBLEMNAME "s2.upsolve1_via_set"
...
void solve() {
  ...
  vector<ll> posFromTallest(N); std::iota(ALL(posFromTallest), 0);
  std::stable_sort(ALL(posFromTallest), [&](const auto & i, const auto & j) { return h[i] > h[j]; } );
  MSG(posFromTallest); LINESEP1;

  ll result = 0LL;

  set<ll> posSoFar;
  for (auto & pos: posFromTallest) {
    posSoFar.insert(pos);

    auto itNxt=posSoFar.find(pos);
    if (++itNxt != posSoFar.end()) {
      ll nxtTaller=*itNxt;
      result += nxtTaller-pos+1;
    }

    auto itPrv=posSoFar.find(pos);
    if ((itPrv--) != posSoFar.begin()) {
      ll prvTaller=*itPrv;
      result += pos-prvTaller+1;
    }
  }
