/* ========================================
   ...
   * Last Modified : Sun 27 Feb 2022 12:17:48 AM CET
...
#define PROBLEMNAME "s3.2.upsolve"
...
void solve() {
  ...
  vector<int> statusCow(N, CLEAN);
  vector<bool> cerealEaten(M);
  queue<ll> nonTreeEdges;
  queue<ll> treeEdges;

  vector<bool> cerealVisited(M);
  function<void(ll)> dfs = [&](ll cereal) {
    cerealVisited[cereal] = true;

    // TODO
    for (auto & [ne, id]: adj[cereal]) {
      if (statusCow[id]==BANNED) {
        continue;
      }
      ...
      if (!cerealVisited[ne]) {
        ...
        if (id!=treeEdges.back()) {
          nonTreeEdges.push(ne);
        }
      }
    }
  };
  ...
  REP(cereal,M) {
    ...
    while (!treeEdges.empty()) {
      result.PB(treeEdges.front());
      treeEdges.pop();
    }
    while (!nonTreeEdges.empty()) {
      result.PB(nonTreeEdges.front());
      nonTreeEdges.pop();
    }
  }

  ll nHungry = std::accumulate(ALL(statusCow), 0LL, [&](const auto & s, const auto & x) { return s+(x==EAT); } );
  cout << nHungry << endl;
  for (auto & cow: result) {
    cout << cow << endl;
  }
