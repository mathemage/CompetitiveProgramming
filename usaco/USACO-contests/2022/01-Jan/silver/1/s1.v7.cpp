/* ========================================
   ...
   * Last Modified : Tue 01 Feb 2022 12:38:34 AM CET
...
//   assert(SZ(bots)<=3*50);
  ...
  ll result = INF_LL;
//   unordered_map<ll, ll> dist;   // memo
//   map<ll, ll> dist;   // memo
  for (auto & [d, candidate]: bots) {
    if (candidate>end) {
      continue;
    }
    LINESEP1;
...
//     set<ll> vis;
    ...
    while (!q.empty()) {
      ...
      for (auto & ne: neigh) {
        if (ne>=candidate && !CONTAINS(dist, ne)) {
          dist[ne]=dist[at]+1;
//           vis.insert(ne);
          q.push(ne);
...
//     assert(CONTAINS(dist, candidate));
    umin(result, d+dist[candidate]);
  }
  cout << result << endl;
}
