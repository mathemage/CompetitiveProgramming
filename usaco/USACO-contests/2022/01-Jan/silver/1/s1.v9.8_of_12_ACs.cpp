/* ========================================
   ...
   * Last Modified : Tue 01 Feb 2022 12:51:04 AM CET
   * Created By : Karel Ha <mathemage@gmail.com>
   * URL : http://usaco.org/current/index.php?page=viewproblem&cpid=1170
   * Points/Time :
   * Total/ETA :
   * Status :
   * 1/12 ACs, 11/12 TLEs :-/
   * 4/12 ACs, 8/12 TLEs ^_^
   * 4/12 ACs, 8/12 REs/MLEs :-/
   * 4/12 ACs, 8/12 REs/MLEs :-/
   * 4/12 ACs, 8/12 REs/MLEs :-/ :-/
   * 4/12 ACs, 8/12 REs/MLEs :-/ :-/
   * 4/12 ACs, 8/12 TLEs :-(
   * 4/12 ACs, 8/12 TLEs :-( :-(
   * 8/12 ACs, 4/12 TLEs :-O :-O
   * -> last minute reward!! ^_^
   *
   ==========================================*/

#include <queue>
#define PROBLEMNAME "s1"
...
void solve() {
  ...
  MSG(start); MSG(end); LINESEP1;

  unordered_set<ll> candidates;
  unordered_map<ll, ll> candidateDists;
  ll d=0;
  ll val=start;
  while (val>1) {
    candidates.insert(val);
    candidateDists[val]=d;
    if (val%2==0) {
      val/=2;
    } else {
      val++;
  ...
  while (!q.empty()) {
    ...
    if (CONTAINS(candidates, at)) {
      umin(result, candidateDists[at]+dist[at]);
//       if (++nSolved==SZ(candidates)) {
      if (++nSolved>=SZ(candidates)/2) {
        break;
      }
    }

    vector<ll> neigh;
    if (at-1>=1) { neigh.PB(at-1); }
    if (at%2==0) { neigh.PB(at/2); }
