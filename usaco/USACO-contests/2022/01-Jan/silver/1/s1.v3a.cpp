/* ========================================
   ...
   * Last Modified : Mon 31 Jan 2022 09:41:47 PM CET
...
void solve() {
  ...
  while (!q.empty()) {
    ...
    vector<ll> neigh{at+1}; // TODO check for overflow
    if (at%2==0) { neigh.PB(at/2); }

    // heuristics
    if (at<end) { neigh.PB(2*at); }

    for (auto & ne: neigh) {
      if (!CONTAINS(dist, ne)) {
        dist[ne]=dist[at]+1;
        q.push({get_dist(ne), ne});
      }
    }
  }

  assert(CONTAINS(dist, end));
  ll result = dist[end];
  cout << result << endl;
}
