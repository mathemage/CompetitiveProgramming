/* ========================================
   ...
   * Last Modified : Sat 29 Jan 2022 03:02:58 PM CET
   * Created By : Karel Ha <mathemage@gmail.com>
   * URL : http://usaco.org/index.php?page=viewproblem&cpid=1167
   * Points/Time :
   * Total/ETA :
   * Status :
   * 11/11 ACs (passed & flashed!)
   *
   ==========================================*/
...
void solve() {
  vector<string> ans(3); cin >> ans;
  vector<string> guess(3); cin >> guess;
  MSG(ans); MSG(guess); LINESEP1;
  
  unordered_map<char, ll> ansFreq;
  for (auto & line: ans) {
    for (auto & ch: line) {
      ansFreq[ch]++;
    }
  }
  unordered_map<char, ll> guessFreq;
  for (auto & line: guess) {
    for (auto & ch: line) {
      guessFreq[ch]++;
    }
  }
  MSG(ansFreq); MSG(guessFreq); LINESEP1;

  ll green = 0LL;
  REP(r,3) {
    REP(c,3) {
      if (ans[r][c]==guess[r][c]) {
        green++;
        ansFreq[ans[r][c]]--;
        guessFreq[guess[r][c]]--;
      }
  ...
  ll yellow = 0LL;
  for (auto & [ch, cnt]: ansFreq) {
    yellow += min(cnt, guessFreq[ch]);
  }
