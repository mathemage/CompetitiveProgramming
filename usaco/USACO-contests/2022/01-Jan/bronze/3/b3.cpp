/* ========================================
   ...
   * Last Modified : Sat 29 Jan 2022 05:49:04 PM CET
   * Created By : Karel Ha <mathemage@gmail.com>
   * URL : http://usaco.org/index.php?page=viewproblem&cpid=1169
   * Points/Time :
   * Total/ETA :
   * Status :
   * 15/15 ACs (passed & flashed!!) -> ICPed!!!
   *
   ==========================================*/

#define PROBLEMNAME "b3"
...
void solve() {
  ...
  REP(_,2) {
    ...
    vector<ll> d(dSz);
    auto get_d = [&](ll i) {
      return (0<=i && i<dSz) ? d[i] : 0;
    };
    MSG(d); LINESEP1;

    FOR(i,1,dSz-1) {
      d[i] = h[i]-h[i-1]+get_d(i-2);
    }
    MSG(d); LINESEP1;

#warning What about the last d[N-1]?

    // shift even pos by minOffset
    ll minOff=INF_LL;
    for (ll i = 0; i < dSz; i += 2) {
      umin(minOff, d[i]);
    }
    MSG(minOff);
    for (ll i = 0; i < dSz; i += 2) {
  ...
  cout << (results.front()==results.back() ? results[0] : -1) << endl;
}
