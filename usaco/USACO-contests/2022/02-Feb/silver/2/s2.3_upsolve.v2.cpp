/* ========================================
   ...
   * Last Modified : Fri 11 Mar 2022 12:20:40 AM CET
...
void solve() {
  ...
  ll N1=N/2;
  ll N2=N-N1;
  unordered_map<ll, unordered_map<ll, unordered_map<ll, ll>>> xyk2w[2];
  ...
  REP(iN,2) {
    REP(b,1LL<<Ns[iN]) {
      ...
      xyk2w[iN][x][y][__builtin_popcountll(b)]++;
    }
  }

  vector<ll> result(N+5);
  for (auto & [x, yk2w]: xyk2w[0]) {
    for (auto & [y, k2w]: yk2w) {
      for (auto & [k, w]: k2w) {
        for (ll k2 = 0; k+k2 <= N; k2 += 1) {
          if (CONTAINS(xyk2w[1], xg-x)
           && CONTAINS(xyk2w[1][xg-x], yg-y)
           && CONTAINS(xyk2w[1][xg-x][yg-y], k2) ) {
            result[k+k2] += w * xyk2w[1][xg-x][yg-y][k2];
          }
        }
      }
    }
