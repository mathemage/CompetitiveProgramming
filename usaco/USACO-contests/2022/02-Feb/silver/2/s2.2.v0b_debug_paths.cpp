/* ========================================
   ...
   * Last Modified : Sun 27 Feb 2022 06:09:20 PM CET
...
void solve() {
  ...
  unordered_map<ll, unordered_map<ll, unordered_map<ll, ll>>> ways;
  ways[0][0][0]=1LL;

#ifdef MATHEMAGE_DEBUG
  unordered_map<ll, unordered_map<ll, unordered_map<ll, vector<vector<ll>>>>> paths;
  paths[0][0][0].PB({});
#endif
  ...
  REP(i,N) {
    ...
    for (auto & [x, ykw]: ways) {
      for (auto & [y, kw]: ykw) {
        for (auto & [k,w]: kw) {
          ways[x+xs[i]][y+ys[i]][k+1] += w;
#ifdef MATHEMAGE_DEBUG
          for (auto p: paths[x][y][k]) {
            p.PB(i+1);
            paths[x+xs[i]][y+ys[i]][k+1].PB(p);
          }
#endif

          if (k==3 && x+xs[i]==xg && y+ys[i]==yg) {
            MSG(x); MSG(y); MSG(k); MSG(w);
            MSG_VEC_VEC(paths[x][y][k]);
            MSG_VEC_VEC(paths[x+xs[i]][y+ys[i]][k+1]);
            LINESEP1;
          }
        }
      }
