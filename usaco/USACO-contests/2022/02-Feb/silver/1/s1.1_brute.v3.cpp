/* ========================================
   ...
   * File Name : s1.1_brute.cpp
   * Creation Date : 27-02-2022
   * Last Modified : Sun 27 Feb 2022 04:01:57 PM CET
...
void solve() {
  ...
  vector<vector<bool>> cow2okGifts(N, vector<bool>(N));
  vector<vector<ll>> gift2order(N, vector<ll>(N));
  REP(i,N) {
    bool ok=1;
    REP(j,N) {
      preferences[i][j]--;
      gift2order[i][preferences[i][j]] = j;

      cow2okGifts[i][preferences[i][j]]=ok;
  ...
  MSG_VEC_VEC(gift2order);
  MSG_VEC_VEC(cow2okGifts);
  LINESEP2;

  vector<ll> result(N); std::iota(ALL(result), 0);

  vector<ll> perm(N); std::iota(ALL(perm), 0);
  ll countDown=5e8/N;
  do {
    bool ok=1;
    REP(i,N) {
      if (!cow2okGifts[i][perm[i]]) {
        ok=0;
        break;
      }
    }
    ...
    if (ok) {
      REP(i,N) {
        if (gift2order[i][perm[i]] < gift2order[i][result[i]]) {
          result[i] = perm[i];
        }
      }
    }
  } while (--countDown>=0 && next_permutation(ALL(perm)));
