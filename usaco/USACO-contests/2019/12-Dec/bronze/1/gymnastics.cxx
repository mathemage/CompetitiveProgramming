/* ========================================
   ...
   * File Name : gymnastics.cxx
   * Creation Date : 15-07-2021
   * Last Modified : Thu 15 Jul 2021 11:42:37 PM CEST
...
void solve() {
  ll K,N; cin >> K >> N;
  MSG(K); MSG(N); LINESEP1;

  vector<vector<ll>> m(N, vector<ll>(N, UNDEF));
  MSG_VEC_VEC(m); LINESEP1;

  ll before, after;
  REP(_,K) {
    vector<long long> ranking(N); cin >> ranking;
    MSG(ranking);
    ...
    REP(i,N) {
      FOR(j,i+1,N-1) {
        ...
        if (m[before][after]==UNDEF) {
          m[before][after] = 1;
        } else if (m[before][after]!=1) {
          m[before][after] = 0;
        }
