/* ========================================
   ...
   * File Name : buckets.cxx
   * Creation Date : 01-06-2021
   * Last Modified : Tue 01 Jun 2021 09:28:35 PM CEST
...
void solve() {
  vector<string> layout(10, string(10,'?')); cin >> layout;
  MSG(layout);

  queue<pair<int, int>> q;
  vector<vector<int>> dist(10, vector<int>(10,CLEAN));
  ...
  while (!q.empty()) {
    ...
    for (auto & drc: DXY4) {
      ...
      if (bounded(rr,10) && bounded(cc,10)
          && dist[rr][cc]==CLEAN && layout[rr][cc]!='R') {
        q.push(MP(rr,cc));
  ...
  ll result = dist[target.F][target.S]-1;
  cout << result << endl;
}
