...
/* ========================================
   ...
   * Last Modified : Pá 25. června 2021, 00:02:58
...
void solve() {
  ...
  vector<vector<bool>> config(3, vector<bool>(3));
  REP(i,3) { config[i][i]=1; }
  MSG_VEC_VEC(config); LINESEP1;

  vector<long long> scores(3);
  ...
  REP(_,N) {
    ...
    REP(i,3) {
      MSG(i);
      swap(config[i][a-1], config[i][b-1]); 
      scores[i]+=config[i][g-1];
    }

    LINESEP1;
  }

  ll result = *max_element(ALL(scores));
  cout << result << endl;
}
