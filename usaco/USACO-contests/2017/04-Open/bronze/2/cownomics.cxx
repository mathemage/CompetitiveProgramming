/* ========================================
   ...
   * File Name : cownomics.cxx
   * Creation Date : 27-07-2021
   * Last Modified : Tue 27 Jul 2021 12:13:48 AM CEST
...
void solve() {
  ll N,M; cin >> N >> M;
  vector<string> spotty(N); cin >> spotty;
  vector<string> plain(N); cin >> plain;
  MSG(spotty); MSG(plain);
  LINESEP1;

  ll result = 0LL;
  REP(pos,M) {
    set<char> spottyGenes;
    REP(i,N) {
      spottyGenes.insert(spotty[i][pos]);
    }

    bool ok=1;
    REP(i,N) {
      if (CONTAINS(spottyGenes, plain[i][pos])) {
        ok=0;
        break;
      }
    }
