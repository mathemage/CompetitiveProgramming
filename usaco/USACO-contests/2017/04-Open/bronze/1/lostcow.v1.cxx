/* ========================================
   ...
   * File Name : lostcow.cxx
   * Creation Date : 20-07-2021
   * Last Modified : Tue 20 Jul 2021 12:36:06 AM CEST
...
void solve() {
  ...
  ll reach=1LL;
  while (reach<INF) {
    MSG(start); MSG(sign); MSG(reach); 

    for (ll pos = start; pos != start+sign*(reach+1); pos += sign) {
      MSG(pos);
      if (pos==y) {
        cout << result << endl;
        return;
