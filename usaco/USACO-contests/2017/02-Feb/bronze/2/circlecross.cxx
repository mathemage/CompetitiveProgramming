/* ========================================
   ...
   * File Name : circlecross.cxx
   * Creation Date : 06-11-2021
   * Last Modified : Sat 06 Nov 2021 12:17:39 AM CET
...
void solve() {
  ...
  vector<vector<ll>> ch2pos(26);
  for (ll i = 0; i < SZ(S); i += 1) {
    ch2pos[S[i]-'A'].PB(i);
  }
  MSG_VEC_VEC(ch2pos); LINESEP1;

  ll result = 0LL;
  REP(a,26) {
    FOR(b,a+1,25) {
      for (auto & [i,j]: {MP(a,b), MP(b, a)}) {
        if (ch2pos[i][0]<ch2pos[j][0]
         && ch2pos[j][0]<ch2pos[i][1]
         && ch2pos[i][1]<ch2pos[j][1] ) {
          result++;
          break;
        }
      }
