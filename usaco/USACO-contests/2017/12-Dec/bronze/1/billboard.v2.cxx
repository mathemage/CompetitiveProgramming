/* ========================================
   ...
   * Last Modified : Sat 12 Jun 2021 09:50:50 PM CEST
...
void solve() {
  vector<vector<long long>> xy(3, vector<long long>(4));
  cin >> xy;
  MSG_VEC_VEC(xy);

  ll result = 0LL;
  for (auto & billboard: {0,1}) {
    ll dx=xy[billboard][2]-xy[billboard][0];
    ll dy=xy[billboard][3]-xy[billboard][1];
    result+=dx*dy;

    // -= intersection area
    ll dxIntersect=min(xy[billboard][2],xy[2][2]) - max(xy[billboard][0],xy[2][0]);
    umax(dxIntersect,0LL);
    ll dyIntersect=min(xy[billboard][3],xy[2][3]) - max(xy[billboard][1],xy[2][1]);
    umax(dyIntersect,0LL);
    result-=dxIntersect*dyIntersect;
  }
