/* ========================================
   ...
   * Last Modified : Sat 12 Jun 2021 09:41:17 PM CEST
   ...
   * unsubmitted
   * - completely wrong
   *   - mistaken no. of cells with surface area :-/
   *
   ==========================================*/
...
void solve() {
  vector<vector<long long>> xy(3, vector<long long>(4)); cin >> xy;
  MSG_VEC_VEC(xy);

  ll result = 0LL;
//   FOR(x,-1000,1000) {  // TODO
//     FOR(y,-1000,1000) {
  FOR(x,-10,10) {
    FOR(y,-10,10) {
//       MSG(x); MSG(y);
      ...
      ok |= xy[0][0]<=x && x<=xy[0][2]
         && xy[0][1]<=y && y<=xy[0][3];

      ok |= xy[1][0]<=x && x<=xy[1][2]
         && xy[1][1]<=y && y<=xy[1][3];

      ok &= !(xy[2][0]<=x && x<=xy[2][2]
           && xy[2][1]<=y && y<=xy[2][3]);
