/* ========================================
   ...
   * File Name : bronze3.cpp
   * Creation Date : 17-12-2021
   * Last Modified : Fri 17 Dec 2021 11:58:07 PM CET
   * Created By : Karel Ha <mathemage@gmail.com>
   * URL : /home/mathemage/src/github/mathemage/CompetitiveProgramming/usaco/USACO-contests/2020/12-Dec/bronze/3/bronze3.cpp
   * Points/Time :
   * = 38m
   *
   * Total/ETA :
   * Status :
   * 10/10 ACs (passed) - uffff, yessss!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
   *
   ==========================================*/

#define PROBLEMNAME "bronze3"
...
void solve() {
  ...
  REP(_,N-1) {
    ...
    REP(from,N) {
      ...
      REP(to,N) {
        if (faces[from]!=faces[to] && start[from]<fixed[to]
            && fixed[from]-start[to]!=fixed[to]-start[from]) {
          if (   (end[to]<INF_LL && start[to]<=fixed[from] && fixed[from]<=end[to])
              || (end[to]==INF_LL && start[to]<=fixed[from]
                    && fixed[from]-start[to]<fixed[to]-start[from]) ) {
            if (umin(minTime, fixed[to]-start[from]-1)) {
              bestI=from;
            }
          }
        }
    ...
    end[bestI]=start[bestI]+minTime;
    umin(result[bestI], end[bestI]-start[bestI]+1);
    MSG(start[bestI]); MSG(end[bestI]); MSG(result[bestI]);
