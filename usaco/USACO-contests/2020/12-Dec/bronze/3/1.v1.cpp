/* ========================================
   ...
   * Last Modified : Pá 17. prosince 2021, 00:04:37
...
void solve() {
  ...
  MSG(faces); MSG(pos); LINESEP1;

  vector<ll> result(N, INF_LL);

  while (SZ(north) && SZ(east)) {
    ll minTime = INF_LL;
    ll bestI = UNDEF;
  ...
  for (auto & ans: result) {
    cout << (ans == INF_LL ? "Infinity" : to_string(ans)) << endl;
  }
}
