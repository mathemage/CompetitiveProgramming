/* ========================================
   ...
   * File Name : square.cxx
   * Creation Date : 21-06-2021
   * Last Modified : Mon 21 Jun 2021 12:06:56 AM CEST
   * Created By : Karel Ha <mathemage@gmail.com>
   * URL : http://www.usaco.org/index.php?page=viewproblem2&cpid=663
   * Points/Time : 12m30s
   * Total/ETA :
   * Status : 10/10 ACs -> passed (flash) !
   ==========================================*/
...
void solve() {
  vector<vector<long long>> xy(2, vector<long long>(4)); cin >> xy;

  ll dx=max(xy[0][2], xy[1][2]) - min(xy[0][0], xy[1][0]);
  ll dy=max(xy[0][3], xy[1][3]) - min(xy[0][1], xy[1][1]);
  ll side=max(dx,dy);
  ll result = side*side;
  cout << result << endl;
}
