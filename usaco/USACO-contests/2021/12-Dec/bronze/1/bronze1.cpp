/* ========================================
   ...
   * File Name : bronze1.cpp
   * Creation Date : 18-12-2021
   * Last Modified : Sat 18 Dec 2021 03:14:41 PM CET
   * Created By : Karel Ha <mathemage@gmail.com>
   * URL : http://usaco.org/index.php?page=viewproblem&cpid=1143
   * Points/Time :
   * Total/ETA :
   * Status :
   * 3/11 ACs :-/
...
void solve() {
  ...
  posG.PB(N);
  posH.PB(N);
  MSG(posG); MSG(posH); LINESEP1;

  ll prev,nxt;
  ll result = 0LL;
  for (ll i = 1; i+1 < SZ(posG); i += 1) {
    prev = posG[i]-posG[i-1]-1;
    nxt = posG[i+1]-posG[i]-1;
    result += prev * nxt;
    result += max(0LL, prev-1);
    result += max(0LL, nxt-1);
  }
  for (ll i = 1; i+1 < SZ(posH); i += 1) {
    prev = posH[i]-posH[i-1]-1;
    nxt = posH[i+1]-posH[i]-1;
    result += prev * nxt;
    result += max(0LL, prev-1);
    result += max(0LL, nxt-1);
  }
