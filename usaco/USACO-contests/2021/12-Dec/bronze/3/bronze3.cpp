/* ========================================
   ...
   * Last Modified : Sat 18 Dec 2021 04:31:26 PM CET
   * Created By : Karel Ha <mathemage@gmail.com>
   * URL : http://usaco.org/index.php?page=viewproblem&cpid=1145
   * Points/Time :
   * Total/ETA :
   * Status :
   * 10/10 ACs (passed & flashed!!) - yesssss!!!!!!!!!!!!!!!!!!!!!!!!!!
   *
   ==========================================*/
...
void solve() {
  ...
  REP(r,N) {
    REP(c,N) {
      ...
      FOR(k,0,K) {
        if (r-1>=0 && grid[r-1][c]!='H') {
          if (r-1==0 && c==0) {
            ways[r][c][k][DOWN] += ways[r-1][c][k][NONE];
          } else {
            ways[r][c][k][DOWN] += ways[r-1][c][k][DOWN];
            if (k-1>=0) { ways[r][c][k][DOWN] += ways[r-1][c][k-1][RIGHT]; }
          }
        }

        if (c-1>=0 && grid[r][c-1]!='H') {
          if (r==0 && c-1==0) {
            ways[r][c][k][RIGHT] += ways[r][c-1][k][NONE];
          } else {
            ways[r][c][k][RIGHT] += ways[r][c-1][k][RIGHT];
            if (k-1>=0) { ways[r][c][k][RIGHT] += ways[r][c-1][k-1][DOWN]; }
          }
        }
      }
    }
