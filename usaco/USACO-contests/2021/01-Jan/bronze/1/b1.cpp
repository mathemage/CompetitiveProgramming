/* ========================================
   ...
   * Last Modified : Sun 23 Jan 2022 08:12:37 PM CET
...
void solve() {
  string cowphabet, song; cin >> cowphabet >> song;
  MSG(cowphabet); MSG(song); LINESEP1;

  unordered_map<char, ll> ch2pos;
  for (ll i = 0; i < SZ(cowphabet); i += 1) {
    ch2pos[cowphabet[i]]=i;
  }

  ll result = 1LL;
  for (ll i = 1; i < SZ(song); i += 1) {
    result += ch2pos[song[i]]<=ch2pos[song[i-1]];
  }
  cout << result << endl;
}
