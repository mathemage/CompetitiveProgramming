/* ========================================
   ...
   * Last Modified : Wed 22 Sep 2021 01:08:04 AM CEST
   * Created By : Karel Ha <mathemage@gmail.com>
   * URL : https://contest.yandex.ru/contest/29150/problems/B/
   * Points/Time :
   * = 12m
   *
   * Total/ETA : 3pts
...
const string CLEAN = "ZZZZZ";

void solve() {
  string s; cin >> s;

  string result = CLEAN;
  for (ll i = 0; i+1 < SZ(s); i += 1) {
    if (s[i]==s[i+1]) {
      if (result==CLEAN || s.substr(i,2)<result) {
        result=s.substr(i,2);
      }
    }
  }
  ...
  if (result==CLEAN) {
    for (ll i = 0; i+2 < SZ(s); i += 1) {
      if (s[i]==s[i+2]) {
        if (result==CLEAN || s.substr(i,3)<result) {
          MSG(result); MSG(s.substr(i,3));
          result=s.substr(i,3);
          MSG(result);
        }
      }
  ...
  cout << (result==CLEAN ? "-1" : result) << endl;
}
