/* ========================================
   ...
   * Last Modified : Sat 05 Feb 2022 04:22:36 PM CET
...
void solve() {
  ...
  string line, token;
  set<ip_t, decltype(cmp)> pool(cmp);
  set<ip_t, decltype(cmp)> newPool(cmp);
  REP(_,n) {
    cin >> line;
    MSG(line);
...
//       MSG(octet);
    ...
    pool.insert({addr, mask});
    LINESEP1;
  }

//   std::stable_sort(ALL(pool), [&](const auto & a, const auto & b) { return a.F.to_string() < b.F.to_string(); } );
  MSG(pool);
  LINESEP1;
