/* ========================================
   ...
   * Last Modified : Sat 05 Feb 2022 05:57:46 PM CET
...
//       MSG(octet);
  ...
  while (--countDown>0) {
    ...
    for (ll i = 0; i < SZ(pool); i += 1) {
      ...
      if (i+1<SZ(pool) && mask>0) {
        i_t addrNxt = pool[i+1].F;
        int maskNxt = pool[i+1].S;
        MSG(addrNxt); MSG(maskNxt);

        i_t pref=addr>>(32-mask);
        i_t prefNxt=addrNxt>>(32-maskNxt);
        MSG(pref); MSG(prefNxt);

        if (pref == prefNxt
            && addr[mask]==0 && addrNxt[mask]==1) {
          MSG("merge");
          MSG(addr); MSG(addrNxt);
          LINESEP1;
