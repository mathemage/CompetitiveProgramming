/* ========================================
   ...
   * Last Modified : Út 4. dubna 2023, 19:02:48
...
const ll MX_MEMO = 17;
const ll MX = pow(10,MX_MEMO);
map<ll, ll> forbidden_memo;

const ll FLAG_FORBIDDEN=-1;

ll count_forbidden(ll x) {
...
//   while ((hi-lo)/2>0) {
  while (hi>=lo) {
    ...
    ll cnt_forbidden = count_forbidden(mid); 
    ll count = mid - abs(cnt_forbidden);
    MSG(count);
    assert(count>0);
    if (count==k) {
      while (cnt_forbidden < 0) {
        mid--;
        cnt_forbidden = count_forbidden(mid); 
      }
      count = mid - cnt_forbidden;
//       assert(count == k);
      break;
    }
...
//   setIO(PROBLEMNAME);
  ...
  forbidden_memo[1]=0;
  forbidden_memo[10]=1;
  for (ll pow = 100; pow < MX; pow *= 10) {
    ll prev_pow = pow/10;
    forbidden_memo[pow] = 9*forbidden_memo[prev_pow] + prev_pow;
  }
  MSG(forbidden_memo);
