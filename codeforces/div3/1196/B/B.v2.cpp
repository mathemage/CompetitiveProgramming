/* ========================================
   ...
   * Last Modified : Wed 05 Jan 2022 09:25:28 PM CET
...
//   assert(SZ(result)==k);
  ...
  MSG(result[k-1]);
  ll start = k-2>=0 ? result[k-2] : 0;
  bool ok = std::accumulate(a.begin()+start, a.end(), 0LL, [&](const auto & s, const auto & x) { return (s+x)%2; } );
  ok &= SZ(result)==k;
  cout << ((ok)?"YES":"NO") << endl;

  if (ok) {
    assert(result.back()==n);
    cout << result << endl;
  }
}
