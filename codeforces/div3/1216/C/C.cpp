/* ========================================
   ...
   * Last Modified : Wed 30 Jun 2021 08:20:06 PM CEST
...
void solve() {
  vector<ll> white(4); cin >> white;
  vector<vector<ll>> black(2,  vector<ll>(4)); cin >> black;
  MSG(white);
  MSG_VEC_VEC(black);
  LINESEP1;

  vector<vector<ll>> intersect(2,  vector<ll>(4));
  REP(i,2) {
    REP(di,4) {
      intersect[i][di] = di<2 ? max(white[di],black[i][di]) : min(white[di],black[i][di]);
    }
  }
  MSG_VEC_VEC(intersect);
  LINESEP1;

  vector<ll> interInter(4);
  REP(di,4) {
    interInter[di] = di<2 ? max(intersect[0][di],intersect[1][di]) : min(intersect[0][di],intersect[1][di]);
  }
  MSG(interInter);
  LINESEP1;

  auto area = [](vector<ll> r) {
    return max(0LL, r[2]-r[0]) * max(0LL, r[3]-r[1]);
  };

  MSG(area(intersect[0]) + area(intersect[1]));
  MSG(area(interInter));
  MSG(area(white));
  bool result = area(intersect[0]) + area(intersect[1]) - area(interInter) < area(white);
  YN(result);
}
