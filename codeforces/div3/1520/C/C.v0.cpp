/* ========================================
   ...
   * Last Modified : Wed 05 May 2021 05:20:53 PM CEST
...
bool search(int pos=1) {
  ...
  FOR(d,1,n*n) {
    if (!used[d]) {
      bool d_ok=true;
      for (auto & dxy: DXY4) {
        int rr=r+dxy.F; int cc=c+dxy.S;
        MSG(r) MSG(c); MSG(rr) MSG(cc);
        MSG(r) MSG(c); MSG(rr) MSG(cc);
        MSG(m[r][c]);
        LINESEP1;

        if (bounded(rr,n) && bounded(cc,n) && abs(m[rr][cc]-m[r][c])==1) {
          d_ok=false;
          break;
        }
        MSG(m[rr][cc]) 
