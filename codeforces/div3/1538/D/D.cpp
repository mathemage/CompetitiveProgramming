/* ========================================
   ...
   * Last Modified : Thu 10 Jun 2021 06:50:30 PM CEST
   * Created By : Karel Ha <mathemage@gmail.com>
   * URL : https://codeforces.com/contest/1538/problem/D
   * Points/Time :
   * Total/ETA :
   * Status : unsolved after 1hr+ of trying :-( :-( :-( :-(
   ==========================================*/
...
void solve() {
  ll a,b,k; cin >> a >> b >> k;
  MSG(a); MSG(b); LINESEP1;

  vector<long long> alphaA(SZ(primes));
  vector<long long> alphaB(SZ(primes));
  ...
  for (int i = 0; i < SZ(primes); i += 1) {
    ...
    minK+=alphaA[i]!=alphaB[i];
    maxSingles+=abs(alphaA[i]-alphaB[i]);
  }
  MSG(minK); MSG(maxSingles); LINESEP1;
//   MSG(alphaA); MSG(alphaB); LINESEP1;
...
//   setIO(PROBLEMNAME);
  ...
  primes = eratosthenes_sieve(MAX_PR);
//   primes = eratosthenes_sieve(20);
  MSG(SZ(primes));
  LINESEP2;
