/* ========================================
   ...
   * Last Modified : Thu 25 Mar 2021 05:08:45 PM CET
...
void solve() {
  ...
  FORD(len,szA,1) {
    MSG(len);
    for (int ia = 0; ia+len-1 < szA; ia += 1) {
      string aSub=a.substr(ia, len);
      for (int ib = 0; ib+len-1 < szB; ib += 1) {
        string bSub=b.substr(ib, len);
        if (aSub==bSub) {
          MSG(aSub); MSG(bSub); MSG(SZ(aSub));
          cout << szA+szB-2*SZ(aSub) << endl;
          return;
        }
      }
    }
