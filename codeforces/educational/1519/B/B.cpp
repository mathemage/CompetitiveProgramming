/* ========================================
   ...
   * Last Modified : Mon 03 May 2021 11:13:26 PM CEST
...
void solve() {
  ...
  vector<vector<set<int>>> dp(1+n, vector<set<int>>(1+m));
  dp[1][1]={0};

  FOR(r,1,n) {
    FOR(c,1,m) {
      if (r>1) {
        for (auto & cost: dp[r-1][c]) {
          dp[r][c].insert(cost+c);
        }
      }

      if (c>1) {
        for (auto & cost: dp[r][c-1]) {
          dp[r][c].insert(cost+r);
        }
      }
    }
  }
