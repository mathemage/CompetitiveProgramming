/* ========================================
   ...
   * Last Modified : Fri 18 Jun 2021 12:25:26 AM CEST
   * Created By : Karel Ha <mathemage@gmail.com>
   * URL : https://codeforces.com/contest/1534/problem/C
   * Points/Time :
   *
   * Total/ETA :
   * [coding kata 2 - via union-find] ~12m
   *
   * Status :
   ==========================================*/
...
void solve() {
  ...
  vector<int> link(n+5); iota(ALL(link), 0);
  vector<ll> sizes(n+5);
  ...
  FOR(val,1,n) {
    if (SZ(edges[val]) == 2) {
      unite(edges[val][0], edges[val][1]);
    } else {
      assert(SZ(edges[val]) == 2);
    }
  }

  // product of comp sizes
  vector<long long> compSizes;
  REP(pos,n) {
    if (find(pos)==pos) {
      compSizes.PB(sizes[pos]);
    }
  }

  ll result = accumulate(ALL(compSizes), 0LL, [](auto & a, auto & b) { return a=(a+b%MOD)%MOD; } );
  cout << result << endl;
}
