/* ========================================
   ...
   * Last Modified : Sun 13 Jun 2021 07:28:00 PM CEST
...
void solve() {
  ...
  vector<vector<long long>> val2idx(2, vector<long long>(n, UNDEF));
  vector<vector<long long>> a(2, vector<long long>(n, UNDEF));
  REP(r,2) {
    REP(c,n) {
      cin >> a[r][c];
      val2idx[r][--a[r][c]]=c;
  ...
  vector<bool> vis(n);
  std::function<void(ll)> bfs = [&](ll root) {
    queue<ll> q;
