/* ========================================
   ...
   * Last Modified : Sun 13 Jun 2021 07:28:09 PM CEST
...
void solve() {
  ...
  vector<vector<long long>> val2idx(2, vector<long long>(1+n, CLEAN));
  vector<vector<long long>> a(2, vector<long long>(1+n, CLEAN));
  REP(r,2) {
    REP(c,n) {
      cin >> a[r][c];
      val2idx[r][a[r][c]]=c;
  ...
  MSG_VEC_VEC(val2idx); LINESEP1;

  map<ll, set<long long>> adj;
  REP(at,n) {
    REP(r,2) {
      ll val=a[r][at];
      ll neigh=val2idx[1-r][val];
      adj[at].insert(neigh);
      adj[neigh].insert(at);
    }
  }
  ...
  vector<bool> vis(n);
//   auto dfs = [&](ll at) {
  std::function<void(ll)> dfs = [&](ll at) {
    vis[at]=true;
