/* ========================================
   ...
   * Last Modified : Mon 22 Mar 2021 07:15:26 PM CET
...
void solve() {
  ...
  long long exp, pow;
  for (exp=1, pow=10; pow <= n; exp++, pow*=10) {
    result += (pow-pow/10) * exp;
    MSG(exp) MSG(pow) MSG(result); LINESEP1;
  }
  pow/=10;
  LINESEP1; MSG(exp) MSG(pow) MSG(result); MSG(n-pow+1);
  LINESEP1;
  result += (n-pow+1) * exp;
  cout << result << endl;
}
