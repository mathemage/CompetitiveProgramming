...
/* ========================================
   ...
   * Last Modified : Pá 2. července 2021, 19:15:21
...
void solve() {
  ...
  sort(ALL(aIdx), [](auto & a, auto & b) { return a.S < b.S; } );

  ll result = 0LL;
  REP(i,n) {
    for (ll j = i+1; j<n && aIdx[i].S*aIdx[j].S < 2*n; j += 1) {
      result += aIdx[i].S*aIdx[j].S==aIdx[i].F+aIdx[j].F;
    }
  }
