/* ========================================
   ...
   * Last Modified : Fri 02 Jul 2021 12:49:43 AM CEST
   * Created By : Karel Ha <mathemage@gmail.com>
   * URL : https://codeforces.com/contest/1541/problem/B
   * Points/Time :
   * + 2m (+30-60m hrs of thinking)
   * [hint 1]
   * +17m = 19m :-/
   *
   * Total/ETA :
...
void solve() {
  ...
  REP(i,n) {
    if (a[i]*a[i]<=2*n) {
      REP(j,n) {
        result += i!=j && a[i]*a[j]==i+1+j+1;
        result -= i<j && a[i]*a[j]==i+1+j+1 && a[j]*a[j]<=2*n;  // offset double-counting
      }
    }
  }
  cout << result << endl;
