...
string	lettersGuessed;
		...
		__in >> __expected >> secretWord >> lettersGuessed;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << secretWord << ", " << lettersGuessed << std::endl;
		std::cout.flush();

		__timer_start();

		HangmanHelper __object;
		string __actual = __object.show(secretWord, lettersGuessed);
