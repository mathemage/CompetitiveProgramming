...
#include "DistancesBetweenNumbers.cc"
...
vector <int>	Aprefix;
		...
		__in >> __expected >> N >> Aprefix;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << N << ", " << Aprefix << std::endl;
		std::cout.flush();

		__timer_start();

		DistancesBetweenNumbers __object;
		long long __actual = __object.count(N, Aprefix);
