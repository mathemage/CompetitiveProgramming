...
//     vector<int> A(N);
    ...
    vector<vector<long long>> counter(MAX_DIGITS, vector<long long>(N_DIGITS, 0LL));

    long long result=0LL;
    REP(i,N) {
      ll Ai=A[i];
      string sAi = to_string(Ai);
      while (SZ(sAi)<MAX_DIGITS) { sAi = "0"+sAi; }
      MSG(Ai); MSG(sAi);

      REP(pos,MAX_DIGITS) {
        MSG(result); MSG(pos); MSG(sAi[pos]-'0');
        result+=i-counter[pos][sAi[pos]-'0'];
