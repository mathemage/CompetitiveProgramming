...
int main(int argc, char* argv[])
{
	...
	for(;;)
	{
		...
		__in >> __expected >> today;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << today << std::endl;
		std::cout.flush();

		__timer_start();

		NextOlympics __object;
		int __actual = __object.countDays(today);
