...
string	Gender;
		...
		__in >> __expected >> Dad >> Mom >> FirstChild >> Gender;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << Dad << ", " << Mom << ", " << FirstChild << ", " << Gender << std::endl;
		std::cout.flush();

		__timer_start();

		AqaAsadiNames __object;
		string __actual = __object.getName(Dad, Mom, FirstChild, Gender);
