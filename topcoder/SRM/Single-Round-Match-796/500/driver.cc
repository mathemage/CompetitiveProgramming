...
#include "ChristmasLightsFixing.cc"
...
long long	step;
		...
		__in >> __expected >> N >> step;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << N << ", " << step << std::endl;
		std::cout.flush();

		__timer_start();

		ChristmasLightsFixing __object;
		vector <int> __actual = __object.fixingTime(N, step);
