...
int	E;
		...
		__in >> __expected >> N >> M >> A >> B >> C >> D >> E;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << N << ", " << M << ", " << A << ", " << B << ", " << C << ", " << D << ", " << E << std::endl;
		std::cout.flush();

		__timer_start();

		LunchLine __object;
		long long __actual = __object.simulate(N, M, A, B, C, D, E);
