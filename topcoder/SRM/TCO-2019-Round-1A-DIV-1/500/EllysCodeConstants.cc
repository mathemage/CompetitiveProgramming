...
public:
  string getLiteral(string candidate) {
    ...
    string result="0x";
    int n=SZ(candidate);
    for (int i = 0; i < candidate.size(); i += 1) {
      char ch=candidate[i];
      if ('A'<=ch && ch<='F') { result+=ch; }
      else if (ch=='O') { result+='0'; }
      else if (ch=='I') { result+='1'; }
      else if (ch=='Z') { result+='2'; }
      else if (ch=='S') { result+='5'; }
      else if (ch=='T') { result+='7'; }
      else {
        if (i>=1 && i==n-1 && (ch=='L' || ch=='U')) { result+=ch; }
        else if (i>=1 && i==n-2
            && (candidate.substr(i,2)=="LL" || candidate.substr(i,2)=="UL" || candidate.substr(i,2)=="LU") ) { result+=candidate.substr(i,2); }
        else if (i>=1 && i==n-3
            && (candidate.substr(i,3)=="ULL" || candidate.substr(i,3)=="LLU") ) { result+=candidate.substr(i,3); }
        else {
          return "";
        }
        break;
