...
int main(int argc, char* argv[])
{
	...
	for(;;)
	{
		...
		__in >> __expected >> candidate;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << candidate << std::endl;
		std::cout.flush();

		__timer_start();

		EllysCodeConstants __object;
		string __actual = __object.getLiteral(candidate);
