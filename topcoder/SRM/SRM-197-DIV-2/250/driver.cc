...
int	hoseDist;
		...
		__in >> __expected >> n >> rowDist >> colDist >> hoseDist;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << n << ", " << rowDist << ", " << colDist << ", " << hoseDist << std::endl;
		std::cout.flush();

		__timer_start();

		GardenHose __object;
		int __actual = __object.countDead(n, rowDist, colDist, hoseDist);
