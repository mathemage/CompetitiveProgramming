...
int	columns;
		...
		__in >> __expected >> rows >> columns;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << rows << ", " << columns << std::endl;
		std::cout.flush();

		__timer_start();

		PaintingCells __object;
		int __actual = __object.paint(rows, columns);
