...
int	checks;
		...
		__in >> __expected >> startingBalance >> debits >> checks;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << startingBalance << ", " << debits << ", " << checks << std::endl;
		std::cout.flush();

		__timer_start();

		CheckbookRegister __object;
		int __actual = __object.updateBalance(startingBalance, debits, checks);
