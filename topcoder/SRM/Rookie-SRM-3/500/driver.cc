...
int	maxStones;
		...
		__in >> __expected >> stonePile >> maxStones;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << stonePile << ", " << maxStones << std::endl;
		std::cout.flush();

		__timer_start();

		TakeStones __object;
		string __actual = __object.result(stonePile, maxStones);
