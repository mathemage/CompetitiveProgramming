...
int main(int argc, char* argv[])
{
	...
	for(;;)
	{
		...
		__in >> __expected >> scores;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << scores << std::endl;
		std::cout.flush();

		__timer_start();

		JudgedScoring __object;
		int __actual = __object.overallScore(scores);
