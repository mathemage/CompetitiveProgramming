...
int	iFactor;
		...
		__in >> __expected >> sFactor >> iFactor;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << sFactor << ", " << iFactor << std::endl;
		std::cout.flush();

		__timer_start();

		StringMult __object;
		string __actual = __object.times(sFactor, iFactor);
