...
int	N;
		...
		__in >> __expected >> R >> C >> N;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << R << ", " << C << ", " << N << std::endl;
		std::cout.flush();

		__timer_start();

		MoveAllMarbles __object;
		vector <int> __actual = __object.move(R, C, N);
