...
long long	N;
		...
		__in >> __expected >> H >> V >> N;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << H << ", " << V << ", " << N << std::endl;
		std::cout.flush();

		__timer_start();

		DivideThePlane __object;
		long long __actual = __object.makeCuts(H, V, N);
