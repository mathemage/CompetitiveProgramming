...
public:
  long long makeCuts(int H, int V, long long N) {
    ...
    long long b=V+1;
    MSG(a) MSG(b) 
    for (long long d = 1; d*d <= N+5; d += 1) {
      if (N%d==0) {
        MSG(d) MSG(result);
        MINUPDATE(result, max(0LL, (long long)(d-a))+max(0LL, (long long)((N/d)-b)));
        MINUPDATE(result, max(0LL, (long long)(d-b))+max(0LL, (long long)((N/d)-a)));
        MSG(result);
        LINESEP1;
      }
    }
