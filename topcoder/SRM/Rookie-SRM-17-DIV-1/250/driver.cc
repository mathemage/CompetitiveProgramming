...
int	target;
		...
		__in >> __expected >> cards >> target;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << cards << ", " << target << std::endl;
		std::cout.flush();

		__timer_start();

		OneCardDraw __object;
		double __actual = __object.pickChance(cards, target);
