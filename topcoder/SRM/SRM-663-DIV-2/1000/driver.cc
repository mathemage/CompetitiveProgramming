...
int main(int argc, char* argv[])
{
	...
	for(;;)
	{
		...
		__in >> __expected >> wins;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << wins << std::endl;
		std::cout.flush();

		__timer_start();

		CheeseRolling __object;
		vector<long long> __actual = __object.waysToWin(wins);
