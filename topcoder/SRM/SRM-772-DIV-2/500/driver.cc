...
#include "PermutationAndMultiplication.cc"
...
int	zeroes;
		...
		__in >> __expected >> ones >> zeroes;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << ones << ", " << zeroes << std::endl;
		std::cout.flush();

		__timer_start();

		PermutationAndMultiplication __object;
		int __actual = __object.multiplyAndCount(ones, zeroes);
