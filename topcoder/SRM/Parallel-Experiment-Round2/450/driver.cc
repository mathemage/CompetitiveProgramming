...
int main(int argc, char* argv[])
{
	...
	for(;;)
	{
		...
		__in >> __expected >> text;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << text << std::endl;
		std::cout.flush();

		__timer_start();

		TranspositionKey __object;
		vector <int> __actual = __object.makeKey(text);
