...
vector <int>	clubHi;
		...
		__in >> __expected >> D >> clubLo >> clubHi;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << D << ", " << clubLo << ", " << clubHi << std::endl;
		std::cout.flush();

		__timer_start();

		OptimalGolf __object;
		int __actual = __object.minStrokes(D, clubLo, clubHi);
