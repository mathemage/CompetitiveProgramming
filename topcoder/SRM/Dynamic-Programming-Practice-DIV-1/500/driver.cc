...
int	B;
		...
		std::cout << "# input for test " << __testnum << ": " << M << ", " << B << std::endl;
		std::cout.flush();

		__timer_start();

		MonstersAndBunnies __object;
		double __actual = __object.survivalProbability(M, B);
