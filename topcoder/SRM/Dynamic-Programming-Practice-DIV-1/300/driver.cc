...
int main(int argc, char* argv[])
{
	...
	for(;;)
	{
		...
		__in >> __expected >> smiles;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << smiles << std::endl;
		std::cout.flush();

		__timer_start();

		EmoticonsDiv2 __object;
		int __actual = __object.printSmiles(smiles);
