...
int	maxDist;
		...
		__in >> __expected >> towers >> maxDist;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << towers << ", " << maxDist << std::endl;
		std::cout.flush();

		__timer_start();

		LinearSignals __object;
		int __actual = __object.maxSignals(towers, maxDist);
