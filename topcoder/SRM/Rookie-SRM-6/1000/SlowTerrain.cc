...
public:
  int fastestPath(vector <string> terrain) {
    ...
    REP(r,n) {
      REP(c,m) {
        if (terrain[r][c]=='S') { S=MP(r,c); }
        if (terrain[r][c]=='D') { D=MP(r,c); }
      }
    }
...
//         MSG(nr); MSG(nc);
        ...
        if (!(bounded(nr,n) && bounded(nc,m)) || terrain[nr][nc]=='S') {
          continue;
        }

        ll oldDist=dist[nr][nc];
        if (umin(dist[nr][nc], dist[r][c]+(terrain[nr][nc]=='D'? 0 : terrain[nr][nc]-'0'))) {
          q.erase(MP(oldDist, MP(nr, nc)));
          q.insert(MP(dist[nr][nc], MP(nr, nc)));
        }
