...
int main(int argc, char* argv[])
{
	...
	for(;;)
	{
		...
		__in >> __expected >> terrain;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << terrain << std::endl;
		std::cout.flush();

		__timer_start();

		SlowTerrain __object;
		int __actual = __object.fastestPath(terrain);
