...
public:
  string possible(int n, int k, int x, int y) {
    vector<int> primes=eratosthenes_sieve(1005);
    ...
    auto visitNeigh = [&](int m, int p) {
      for (int mi = p; mi < n; mi += p) {
        if (!vis[mi] && __gcd(mi,m)>k) {
          vis[mi]=1;
          q.push(mi);
        }
      }
    ...
    while (!q.empty()) {
      ...
      int mm=m;
      for (auto & p: primes) {           // iterate over neighbors
        if (p*p>mm) { break; }
