...
public:
  int getNumberOfSpanningTrees(int n, vector <int> x, vector <int> y) {
    ...
    function<bool(int,int)> dfs=[&](int at, int parent) {
      for (auto & neigh: adjList[at]) {
        if (!vis[neigh]) {
          vis[neigh]=1;
          compSize++;
          if (!dfs(neigh, at)) {
        ...
        } else if (neigh!=parent) {  // cycle detection
          return false;
        }
      }
      return true;
    ...
    int result=0;
    REP(i,3) {
      ll count=c[i]*c[(i+1)%3] * c[i]*c[(i+2)%3] % MOD;
      result+=count;
      result%=MOD;
    }
