...
int main(int argc, char* argv[])
{
	...
	for(;;)
	{
		...
		__in >> __expected >> concentration;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << concentration << std::endl;
		std::cout.flush();

		__timer_start();

		CreateMixture __object;
		vector <int> __actual = __object.mix(concentration);
