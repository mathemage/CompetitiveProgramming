...
int main(int argc, char* argv[])
{
	...
	for(;;)
	{
		...
		__in >> __expected >> responses;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << responses << std::endl;
		std::cout.flush();

		__timer_start();

		LogiciansAndBeer __object;
		int __actual = __object.bringBeer(responses);
