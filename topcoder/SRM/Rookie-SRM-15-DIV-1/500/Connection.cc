...
public:
  double signalStrength(vector <int> coeff, vector <int> exp, int dist) {
    ...
    vector<int> ids(SZ(coeff)); std::iota(ALL(ids), 0);
    double result=std::accumulate(ALL(ids), 0.0, [&](const double & s, const int & i) { return s + ((double)coeff[i])*pow(d, (double) exp[i]); } );
    return result;
  }
};
