...
public:
  int distance(vector <string> dictionary, string word) {
    ...
    auto getDist = [&](string s) {
      string w = word;
      int penalty = SZ(s)==SZ(w) ? 0 : 3;
      if (SZ(s)<SZ(w)) {
        swap(s,w);
        penalty = 2;
      }
      ...
      if (abs(SZ(s)-SZ(w)) <= 1) {
        ...
        for (auto & split: splits) {
          int dist = split==INF ? 0 : penalty;
          for (ll i = 0; i < SZ(w); i += 1) {
            dist += (w[i] != s[i + (i>=split)]);
          }
          if (umin(ans, dist)) {
            _D(penalty);
            _D(s); _D(w);
