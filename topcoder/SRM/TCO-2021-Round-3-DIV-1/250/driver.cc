...
int	D;
		...
		__in >> __expected >> U >> L >> D;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << U << ", " << L << ", " << D << std::endl;
		std::cout.flush();

		__timer_start();

		AnnoyingPasswords __object;
		int __actual = __object.count(U, L, D);
