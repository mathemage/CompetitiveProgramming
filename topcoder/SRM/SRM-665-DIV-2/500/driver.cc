...
vector <int>	weight;
		...
		__in >> __expected >> edge1 >> edge2 >> weight;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << edge1 << ", " << edge2 << ", " << weight << std::endl;
		std::cout.flush();

		__timer_start();

		LuckyCycle __object;
		vector <int> __actual = __object.getEdge(edge1, edge2, weight);
