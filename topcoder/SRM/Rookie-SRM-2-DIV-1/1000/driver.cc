...
int main(int argc, char* argv[])
{
	...
	for(;;)
	{
		...
		__in >> __expected >> marbles;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << marbles << std::endl;
		std::cout.flush();

		__timer_start();

		MarbleDrawGame __object;
		double __actual = __object.winningChance(marbles);
