...
vector <int>	Y;
		...
		__in >> __expected >> atoms >> X >> Y;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << atoms << ", " << X << ", " << Y << std::endl;
		std::cout.flush();

		__timer_start();

		OrganicChemistry __object;
		int __actual = __object.countHydrogens(atoms, X, Y);
