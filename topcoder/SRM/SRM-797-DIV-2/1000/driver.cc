...
int	clr;
		...
		__in >> __expected >> R >> C >> H >> cup >> cdn >> clr;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << R << ", " << C << ", " << H << ", " << cup << ", " << cdn << ", " << clr << std::endl;
		std::cout.flush();

		__timer_start();

		FlightPlan __object;
		long long __actual = __object.fly(R, C, H, cup, cdn, clr);
