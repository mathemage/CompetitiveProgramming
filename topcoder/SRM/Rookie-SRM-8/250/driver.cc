...
double	hourlyRate;
		...
		__in >> __expected >> hoursWorked >> hourlyRate;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << hoursWorked << ", " << hourlyRate << std::endl;
		std::cout.flush();

		__timer_start();

		WorkingOvertime __object;
		double __actual = __object.totalPay(hoursWorked, hourlyRate);
