...
#include "AirlineTravelPlanner.cc"
...
vector <string>	dist;
		...
		__in >> __expected >> start >> end >> dist;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << start << ", " << end << ", " << dist << std::endl;
		std::cout.flush();

		__timer_start();

		AirlineTravelPlanner __object;
		int __actual = __object.shortestDistance(start, end, dist);
