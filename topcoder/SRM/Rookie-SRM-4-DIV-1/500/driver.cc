...
int	count;
		...
		__in >> __expected >> marbles >> count;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << marbles << ", " << count << std::endl;
		std::cout.flush();

		__timer_start();

		MarblePicking __object;
		int __actual = __object.fewestColors(marbles, count);
