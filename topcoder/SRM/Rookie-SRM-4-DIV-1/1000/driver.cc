...
double	desiredStrength;
		...
		__in >> __expected >> volume >> solute >> minVolume >> desiredStrength;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << volume << ", " << solute << ", " << minVolume << ", " << desiredStrength << std::endl;
		std::cout.flush();

		__timer_start();

		ChemicalMixing __object;
		double __actual = __object.closestBlend(volume, solute, minVolume, desiredStrength);
