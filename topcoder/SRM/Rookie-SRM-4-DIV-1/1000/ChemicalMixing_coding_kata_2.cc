...
//     vector<vector<bool>> dp(1+sumVol, vector<bool>(1+sumSol));
    vector<vector<int>> dp(1+sumVol, vector<int>(1+sumSol));
    dp[0][0]=1;
    ...
    REP(i,SZ(volume)) {
      FORD(vol,sumVol,volume[i]) {
        FORD(sol,sumSol,solute[i]) {
          if (umax(dp[vol][sol], dp[vol-volume[i]][sol-solute[i]]) && vol>=minVolume) {
            umin(result, abs(desiredStrength - sol/(double)vol ));
          }
        }
      }
    }
