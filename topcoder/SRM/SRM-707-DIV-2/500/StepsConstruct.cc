...
public:
  string construct(vector <string> board, int k) {
    ...
    int n=SZ(board);
    int m=SZ(board[0]);
    MSG(n); MSG(m); MSG_VEC_STR(board); LINESEP1;

    // BFS
    queue<pair<int, int>> q;
    vector<vector<int>> dist(n, vector<int>(m, INF));
    vector<string> prev(n, string(m, '?'));
    ...
    while (!q.empty()) {
      ...
      REP(di,4) {
        ...
        if (bounded(rr,n) && bounded(cc,m) && board[rr][cc]=='.' && dist[rr][cc]==INF) {
          q.push(MP(rr,cc));
          dist[rr][cc]=dist[r][c]+1;
          prev[rr][cc]=directions[di];
        }
      }
    }
    MSG_VEC_VEC(dist); LINESEP1;
    MSG_VEC_STR(prev); LINESEP1;

    if (dist[n-1][m-1]==INF || dist[n-1][m-1]>k) {
      return "";
    }
    ...
    while (!(r==0 && c==0)) {
      ...
      result+=prev[r][c];
      int di=directions.find(prev[r][c]);
      r-=-DY4[di];
      c-=DX4[di];
    }
    reverse(ALL(result));

    // loop tail until !k steps
    string tail="";
    int di=directions.find(result.back());
    tail+=directions[(di+2)%4]; // <- bug here! was %2, should be %4
    tail+=directions[di];
    MSG(di); MSG((di+2)%4);
    MSG(tail); LINESEP1;
    ...
    MSG_VEC_STR(board); LINESEP1;
    MSG(SZ(result));

    return SZ(result)==k ? result : "";
  }
};
