...
public:
  vector <int> sort(vector <string> results) {
    ...
    int N=SZ(results);
    int len=SZ(results[0]);
    MSG(len);

    vector <int> ranking(N);
    std::iota(ALL(ranking), 0);

//     std::stable_sort(ALL(ranking), [&](const auto & a, const auto & b) {
    std::stable_sort(ALL(ranking), [&](int a, int b) {
        string sa=results[a];
        string sb=results[b];
        ...
        for (ll i = len-25; i>=0; i -= 25) {
          LINESEP1;
          MSG(i);
          MSG(sa); MSG(sb);

          string ssa=sa.substr(i,25);
          string ssb=sb.substr(i,25);
          ca=std::count(ALL(ssa), '1');
          MSG(ca);
          cb=std::count(ALL(ssb), '1');
          MSG(cb);
          if (ca!=cb) { return ca>cb; }
        }

        for (ll i = len-1; i>=0; i -= 1) {
          int ca=sa[i];
          int cb=sb[i];
          if (ca!=cb) { return ca>cb; }
        }
