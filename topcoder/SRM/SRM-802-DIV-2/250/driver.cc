...
int main(int argc, char* argv[])
{
	...
	for(;;)
	{
		...
		__in >> __expected >> votes;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << votes << std::endl;
		std::cout.flush();

		__timer_start();

		BallotCounting __object;
		int __actual = __object.count(votes);
