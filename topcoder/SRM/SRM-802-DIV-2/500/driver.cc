...
int main(int argc, char* argv[])
{
	...
	for(;;)
	{
		...
		__in >> __expected >> cave;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << cave << std::endl;
		std::cout.flush();

		__timer_start();

		TheQuestForGold __object;
		string __actual = __object.explore(cave);
