...
int	multiple;
		...
		__in >> __expected >> values >> multiple;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << values << ", " << multiple << std::endl;
		std::cout.flush();

		__timer_start();

		PairedMultiples __object;
		int __actual = __object.selectedTotal(values, multiple);
