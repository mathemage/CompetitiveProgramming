...
#include "PermutationCountsDiv2.cc"
...
vector <int>	pos;
		...
		__in >> __expected >> N >> pos;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << N << ", " << pos << std::endl;
		std::cout.flush();

		__timer_start();

		PermutationCountsDiv2 __object;
		int __actual = __object.countPermutations(N, pos);
