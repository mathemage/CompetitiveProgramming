...
vector <int>	order;
		...
		__in >> __expected >> N >> order;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << N << ", " << order << std::endl;
		std::cout.flush();

		__timer_start();

		AssignLabels __object;
		vector <string> __actual = __object.assign(N, order);
