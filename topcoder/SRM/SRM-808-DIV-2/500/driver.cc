...
vector <int>	p;
		...
		__in >> __expected >> scoreNeeded >> subtasks >> p;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << scoreNeeded << ", " << subtasks << ", " << p << std::endl;
		std::cout.flush();

		__timer_start();

		IOISureWinner __object;
		double __actual = __object.probability(scoreNeeded, subtasks, p);
