/* ========================================
   ...
   * Status :
   * failed systests :-(
   * AC -> yes, beaten up the heisenbug!!
   *
   ==========================================*/
...
//     int MAX=2*n;
    ...
    vector<double> dp(MAX,0.0);
    vector<bool> used(MAX);
    dp[0]=1.0;
    used[0]=1;
//     _D(used); _D(dp); MSG(accumulate(ALL(dp), 0.0)); LINESEP1;
    ...
    REP(i,n) {
      FORD(s,MAX-1,0) {
        ...
        if (s-subtasks[i]>=0 && used[s-subtasks[i]]) {
...
//       _D(used); _D(dp); MSG(accumulate(ALL(dp), 0.0)); LINESEP1;
    ...
    double result=0.0;
    REP(s,min(scoreNeeded,maxSum+1)) {
      result+=dp[s];
      if (scoreNeeded==191) {
        MSG(s); MSG(dp[s]);
        MSG(result); LINESEP1;
    ...
    if (scoreNeeded==191) {
//       _D(used);
      _D(dp); MSG(accumulate(ALL(dp), 0.0));
      MSG(accumulate(dp.begin(), dp.begin()+min(scoreNeeded,maxSum), 0.0));
      MSG(result);
