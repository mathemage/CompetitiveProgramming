...
int main(int argc, char* argv[])
{
	...
	for(;;)
	{
		...
		string	__expected;
		int	N;
vector <string>	openingTime;
vector <string>	closingTime;

		__in >> __expected >> N >> openingTime >> closingTime;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << N << ", " << openingTime << ", " << closingTime << std::endl;
		std::cout.flush();

		__timer_start();

		OpenAllHours __object;
		string __actual = __object.verify(N, openingTime, closingTime);
