...
vector <int>	obsc;
		...
		__in >> __expected >> R >> C >> obsr >> obsc;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << R << ", " << C << ", " << obsr << ", " << obsc << std::endl;
		std::cout.flush();

		__timer_start();

		VisitALot __object;
		string __actual = __object.travel(R, C, obsr, obsc);
