...
int main(int argc, char* argv[])
{
	...
	for(;;)
	{
		...
		__in >> __expected >> prices;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << prices << std::endl;
		std::cout.flush();

		__timer_start();

		AveragePrice __object;
		double __actual = __object.nonDuplicatedAverage(prices);
