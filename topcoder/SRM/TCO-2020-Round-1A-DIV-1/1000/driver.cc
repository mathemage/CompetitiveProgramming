...
int	numItems;
		...
		__in >> __expected >> numSets >> numItems;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << numSets << ", " << numItems << std::endl;
		std::cout.flush();

		__timer_start();

		BlindBoxSets __object;
		double __actual = __object.expectedPurchases(numSets, numItems);
