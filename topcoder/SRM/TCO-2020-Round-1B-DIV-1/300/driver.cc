...
int main(int argc, char* argv[])
{
	...
	for(;;)
	{
		...
		__in >> __expected >> boxes;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << boxes << std::endl;
		std::cout.flush();

		__timer_start();

		EllysCandies __object;
		string __actual = __object.getWinner(boxes);
