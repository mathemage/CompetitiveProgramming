...
vector <int>	r;
		...
		__in >> __expected >> p >> l >> r;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << p << ", " << l << ", " << r << std::endl;
		std::cout.flush();

		__timer_start();

		SegmentsAndPoints __object;
		string __actual = __object.isPossible(p, l, r);
