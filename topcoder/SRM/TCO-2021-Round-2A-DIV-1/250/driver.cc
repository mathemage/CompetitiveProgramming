...
string	volunteersActions;
		...
		__in >> __expected >> exampleGuess >> exampleResponse >> volunteersActions;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << exampleGuess << ", " << exampleResponse << ", " << volunteersActions << std::endl;
		std::cout.flush();

		__timer_start();

		RPSMagicTrick __object;
		string __actual = __object.guess(exampleGuess, exampleResponse, volunteersActions);
