...
public:
  string guess(string exampleGuess, string exampleResponse, string volunteersActions) {
    ...
    int diff=(exampleGuess[1]-exampleGuess[0]+3)%3;
    int flip =
      (diff==1 && exampleResponse=="Wrong.") ||
      (diff==2 && exampleResponse=="Right.");

    vector<string> responses{"AB", "BA"};
    string result;
    for (auto & ch: volunteersActions) {
      if (ch=='?') {
        result += responses[flip];
      } else {
        flip^=1;
      }
    }
