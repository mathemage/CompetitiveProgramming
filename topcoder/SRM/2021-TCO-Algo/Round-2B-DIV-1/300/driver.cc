...
int	seed;
		...
		__in >> __expected >> N >> K >> M >> Pprefix >> seed;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << N << ", " << K << ", " << M << ", " << Pprefix << ", " << seed << std::endl;
		std::cout.flush();

		__timer_start();

		ConstantSegment __object;
		int __actual = __object.sendSomeHome(N, K, M, Pprefix, seed);
