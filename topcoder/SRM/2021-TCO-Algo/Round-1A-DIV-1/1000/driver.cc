...
string	T;
		...
		__in >> __expected >> A >> B >> T;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << A << ", " << B << ", " << T << std::endl;
		std::cout.flush();

		__timer_start();

		EllysRansom __object;
		string __actual = __object.getRansom(A, B, T);
