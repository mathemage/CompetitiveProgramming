...
public:
  vector <int> sort(vector <string> results) {
    ...
    for (int i = 0; i < SZ(results); i += 1) {
      scores[i].F=results[i];
      scores[i].S=i;
    }

    std::sort(ALL(scores), [&](pair<string,int> a, pair<string,int> b) {
        LINESEP1;
        _D(a); _D(b);

        int cmp = SGN(count(ALL(a.F),'1') - count(ALL(b.F),'1'));
        MSG(count(ALL(a.F),'1'));
        MSG(count(ALL(b.F),'1'));
        if (cmp) {
          MSG(cmp);
        ...
        MSG(SZ(a.F));
        for (int i = SZ(a.F); i-25 >= 0; i -= 25) {
          MSG(i); MSG(i-25);
          int cmp=SGN(count(a.F.begin()+i-25,a.F.begin()+i,'1')
...
//     LINESEP1;
    ...
    REP(i,N) {
//       ranking[scores[i].S]=i;
      ranking[i]=scores[i].S;
    }
    return ranking;
  }
};
