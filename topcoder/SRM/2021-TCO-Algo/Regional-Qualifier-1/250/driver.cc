...
int main(int argc, char* argv[])
{
	...
	for(;;)
	{
		...
		__in >> __expected >> results;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << results << std::endl;
		std::cout.flush();

		__timer_start();

		OlympicShooting __object;
		vector <int> __actual = __object.sort(results);
