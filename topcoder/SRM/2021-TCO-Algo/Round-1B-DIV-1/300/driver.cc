...
int main(int argc, char* argv[])
{
	...
	for(;;)
	{
		...
		int	__expected;
		vector <int>	realCitations;
int	budget;
int	citationPrice;

		__in >> __expected >> realCitations >> budget >> citationPrice;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << realCitations << ", " << budget << ", " << citationPrice << std::endl;
		std::cout.flush();

		__timer_start();

		HIndex __object;
		int __actual = __object.cheat(realCitations, budget, citationPrice);
