...
public:
  int cheat(vector <int> realCitations, int budget, int citationPrice) {
    ...
    sort(REVALL(realCitations));
    int maxN=budget/citationPrice;
    int n=SZ(realCitations);

    int hIndex=0;
    REP(i,n) {
      if (realCitations[i]>=i+1) {
        hIndex++;
      } else {
        break;
      }
    }
    MSG_VEC(realCitations); MSG(hIndex); LINESEP1;

    int hTarget=hIndex+1;
    while (hTarget<=n) {
      FORD(i,hTarget-1,0) {
        while (realCitations[i]<(i==hTarget-1? hTarget: realCitations[i+1]) && maxN>0) {
          realCitations[i]++;
          maxN--;
        }
      }
      MSG_VEC(realCitations); MSG(hTarget); LINESEP1;
