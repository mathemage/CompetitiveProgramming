...
string	target;
		...
		__in >> __expected >> parts >> target;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << parts << ", " << target << std::endl;
		std::cout.flush();

		__timer_start();

		BuildString __object;
		string __actual = __object.isPossible(parts, target);
