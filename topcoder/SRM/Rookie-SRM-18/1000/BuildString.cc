...
template<typename T1, typename T2>
inline ostream& operator<<=(ostream& os, const vector<pair<T1, T2>>& vec) { for (const auto & x: vec) {os <<= x; os << " ";} return os; }
...
class BuildString {
public:
  string isPossible(vector <string> parts, string target) {
    LINESEP2;

    ll len=SZ(target);

    unordered_map<ll, bool> memo;
    function<bool(ll)> search = [&](ll start) {
      LINESEP3;
      MSG(start);
      ...
      for (auto & pat: parts) {
        ...
        ll end=start+SZ(pat)-1;
        ll tail=end+1;
        MSG(tail);
        if (end<len && pat==target.substr(start, SZ(pat))) {
          if (!CONTAINS(memo, tail)) {
            memo[tail]=search(tail);
          }
...
* NEVER COPY-PASTE-OVERWRITE, ALWAYS WRITE FRESH/ANEW!!!!

* ONLY COPY-PASTE WHEN DUPLICATING CODE W/O ANY MODIFICATIONS NEEDED!!

* READ THE WHOLE CODE, EVERY LINE, WHEN CHECKING/DEBUGGING!!!
