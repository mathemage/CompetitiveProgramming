...
int	green;
		...
		__in >> __expected >> red >> blue >> green;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << red << ", " << blue << ", " << green << std::endl;
		std::cout.flush();

		__timer_start();

		MarbleBag __object;
		double __actual = __object.getProbability(red, blue, green);
