...
string	color2;
		...
		__in >> __expected >> color1 >> color2;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << color1 << ", " << color2 << std::endl;
		std::cout.flush();

		__timer_start();

		ColorWheel __object;
		string __actual = __object.describePair(color1, color2);
