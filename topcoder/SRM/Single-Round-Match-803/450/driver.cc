...
#include "MarriageAndChargingChallenge.cc"
...
vector <int>	greeting;
		...
		__in >> __expected >> forumLength >> relative >> greeting;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << forumLength << ", " << relative << ", " << greeting << std::endl;
		std::cout.flush();

		__timer_start();

		MarriageAndChargingChallenge __object;
		int __actual = __object.solve(forumLength, relative, greeting);
