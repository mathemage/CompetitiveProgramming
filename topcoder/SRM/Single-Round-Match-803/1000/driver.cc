...
#include "MarriageAndCirclingChallenge.cc"
...
int	state;
		...
		__in >> __expected >> N >> threshold >> state;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << N << ", " << threshold << ", " << state << std::endl;
		std::cout.flush();

		__timer_start();

		MarriageAndCirclingChallenge __object;
		long long __actual = __object.solve(N, threshold, state);
