...
string	c;
		...
		__in >> __expected >> n >> a >> b >> c;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << n << ", " << a << ", " << b << ", " << c << std::endl;
		std::cout.flush();

		__timer_start();

		PalindromePath __object;
		int __actual = __object.shortestLength(n, a, b, c);
