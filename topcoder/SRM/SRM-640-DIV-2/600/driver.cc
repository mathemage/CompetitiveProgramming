...
vector<long long>	table;
		...
		__in >> __expected >> k >> table;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << k << ", " << table << std::endl;
		std::cout.flush();

		__timer_start();

		NumberGameAgain __object;
		long long __actual = __object.solve(k, table);
