...
int	k;
		...
		__in >> __expected >> price >> budget >> k;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << price << ", " << budget << ", " << k << std::endl;
		std::cout.flush();

		__timer_start();

		CucumberMarket __object;
		string __actual = __object.check(price, budget, k);
