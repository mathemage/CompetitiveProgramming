...
int	rightMark;
		...
		__in >> __expected >> leftMark >> rightMark;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << leftMark << ", " << rightMark << std::endl;
		std::cout.flush();

		__timer_start();

		TapeMeasure __object;
		vector <string> __actual = __object.draw(leftMark, rightMark);
