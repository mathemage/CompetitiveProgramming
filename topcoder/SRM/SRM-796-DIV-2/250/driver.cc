...
#include "ChristmasCrackerHappiness.cc"
...
vector <int>	loser;
		...
		__in >> __expected >> N >> winner >> loser;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << N << ", " << winner << ", " << loser << std::endl;
		std::cout.flush();

		__timer_start();

		ChristmasCrackerHappiness __object;
		int __actual = __object.solve(N, winner, loser);
