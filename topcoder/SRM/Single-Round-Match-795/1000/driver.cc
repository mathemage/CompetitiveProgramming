...
int	minDays;
		...
		__in >> __expected >> costs >> minDays;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << costs << ", " << minDays << std::endl;
		std::cout.flush();

		__timer_start();

		AtLeastKDays __object;
		long long __actual = __object.sumOfMinCosts(costs, minDays);
