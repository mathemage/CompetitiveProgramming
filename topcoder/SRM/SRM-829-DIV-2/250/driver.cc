...
int	W;
		...
		__in >> __expected >> N >> A >> W;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << N << ", " << A << ", " << W << std::endl;
		std::cout.flush();

		__timer_start();

		GameShowTotal __object;
		string __actual = __object.verify(N, A, W);
