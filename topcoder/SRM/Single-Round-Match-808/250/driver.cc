...
int main(int argc, char* argv[])
{
	...
	for(;;)
	{
		...
		int	N;
vector <int>	day1scores;
vector <int>	day2scores;

		__in >> __expected >> N >> day1scores >> day2scores;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << N << ", " << day1scores << ", " << day2scores << std::endl;
		std::cout.flush();

		__timer_start();

		IOIGuaranteedPlace __object;
		vector <int> __actual = __object.solve(N, day1scores, day2scores);
