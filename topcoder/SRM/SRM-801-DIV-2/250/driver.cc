...
int	numberOfHouses;
		...
		__in >> __expected >> firstHouse >> numberOfHouses;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << firstHouse << ", " << numberOfHouses << std::endl;
		std::cout.flush();

		__timer_start();

		HouseNumbering __object;
		vector <int> __actual = __object.prepareDigits(firstHouse, numberOfHouses);
