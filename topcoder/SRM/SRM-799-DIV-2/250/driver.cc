...
vector <string>	lastname;
		...
		__in >> __expected >> firstname >> lastname;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << firstname << ", " << lastname << std::endl;
		std::cout.flush();

		__timer_start();

		CoolPairsEasy __object;
		int __actual = __object.count(firstname, lastname);
