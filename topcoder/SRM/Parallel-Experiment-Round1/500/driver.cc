...
int	exponent;
		...
		__in >> __expected >> number >> mantissa >> exponent;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << number << ", " << mantissa << ", " << exponent << std::endl;
		std::cout.flush();

		__timer_start();

		FloatingPoint __object;
		int __actual = __object.representations(number, mantissa, exponent);
