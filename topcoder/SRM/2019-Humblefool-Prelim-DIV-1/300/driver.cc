...
int	numberOfLeaves;
		...
		__in >> __expected >> leaves >> numberOfLeaves;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << leaves << ", " << numberOfLeaves << std::endl;
		std::cout.flush();

		__timer_start();

		SwappingNodes __object;
		vector <int> __actual = __object.swapNodes(leaves, numberOfLeaves);
