...
int	observationDistance;
		...
		__in >> __expected >> raceTime >> observerCount >> observationTime >> observationDistance;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << raceTime << ", " << observerCount << ", " << observationTime << ", " << observationDistance << std::endl;
		std::cout.flush();

		__timer_start();

		WatchedSnail __object;
		double __actual = __object.maxDistance(raceTime, observerCount, observationTime, observationDistance);
