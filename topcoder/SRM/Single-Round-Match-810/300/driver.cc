...
int	N;
		...
		__in >> __expected >> L >> letters >> N;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << L << ", " << letters << ", " << N << std::endl;
		std::cout.flush();

		__timer_start();

		DistinctStrings __object;
		vector <string> __actual = __object.generate(L, letters, N);
