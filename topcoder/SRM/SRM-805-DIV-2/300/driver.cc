...
#include "GalleryIllumination.cc"
...
vector <string>	floorPlan;
		...
		__in >> __expected >> R >> C >> floorPlan;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << R << ", " << C << ", " << floorPlan << std::endl;
		std::cout.flush();

		__timer_start();

		GalleryIllumination __object;
		int __actual = __object.countDarkCells(R, C, floorPlan);
