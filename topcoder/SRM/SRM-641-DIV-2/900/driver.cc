...
int main(int argc, char* argv[])
{
	...
	for(;;)
	{
		...
		__in >> __expected >> permutation;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << permutation << std::endl;
		std::cout.flush();

		__timer_start();

		ShufflingCardsDiv2 __object;
		string __actual = __object.shuffle(permutation);
