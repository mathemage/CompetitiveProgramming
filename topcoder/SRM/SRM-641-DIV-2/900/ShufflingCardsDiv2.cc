...
public:
  string oldShuffle(vector <int> permutation) {
    string res = "";
  ...
  string shuffle(vector <int> permutation) {
    int cnt = 0;
    REP(i,permutation.size()) 
      cnt += (i%2 && permutation[i] <= permutation.size()/2);
    return cnt == permutation.size() / 4 ? "Possible" : "Impossible";
  }
};
