...
inline int rectify(int val) {
  return val>=0 ? val : INF;
}
...
public:
  vector <int> design(vector <int> distances) {
    ...
    vector<int> ord(N); std::iota(ALL(ord), 0);
    _D(ord);

    std::stable_sort(ALL(ord), [&](int i, int j) { return distances[i] < distances[j]; } );
    _D(ord);
    LINESEP1;
    ...
    REP(i,N) {
      ...
      // {number1, length1, number2, length2, number3, length3}.
      ll prev=(at+N-1)%N;
      ll next=(at+1)%N;
      MSG(ord[prev]);
      MSG(ord[at]);
      ...
      MSG(distances[ord[prev]]);
      MSG(distances[ord[at]]);
      MSG(distances[ord[next]]);

      result.PB(ord[prev]);
      result.PB(rectify(distances[ord[at]]-distances[ord[prev]]));

      result.PB(ord[next]);
      result.PB(rectify(distances[ord[next]]-distances[ord[at]]));
