...
int main(int argc, char* argv[])
{
	...
	for(;;)
	{
		...
		__in >> __expected >> distances;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << distances << std::endl;
		std::cout.flush();

		__timer_start();

		MarsHabitats __object;
		vector <int> __actual = __object.design(distances);
