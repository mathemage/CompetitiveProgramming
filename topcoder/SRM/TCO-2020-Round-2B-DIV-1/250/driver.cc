...
#include "FixedNumberOfDigits.cc"
...
long long	numberOfDigits;
		...
		__in >> __expected >> start >> step >> numberOfDigits;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << start << ", " << step << ", " << numberOfDigits << std::endl;
		std::cout.flush();

		__timer_start();

		FixedNumberOfDigits __object;
		long long __actual = __object.sum(start, step, numberOfDigits);
