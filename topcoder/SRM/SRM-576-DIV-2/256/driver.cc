...
vector <int>	leftEnd;
		...
		__in >> __expected >> intensity >> L >> leftEnd;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << intensity << ", " << L << ", " << leftEnd << std::endl;
		std::cout.flush();

		__timer_start();

		TheExperimentDiv2 __object;
		vector <int> __actual = __object.determineHumidity(intensity, L, leftEnd);
