...
int main(int argc, char* argv[])
{
	...
	for(;;)
	{
		...
		__in >> __expected >> individualScores;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << individualScores << std::endl;
		std::cout.flush();

		__timer_start();

		ScoringJudges __object;
		double __actual = __object.overallScore(individualScores);
