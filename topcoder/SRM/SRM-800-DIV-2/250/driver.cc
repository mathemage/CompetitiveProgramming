...
int main(int argc, char* argv[])
{
	...
	for(;;)
	{
		...
		__in >> __expected >> salvagedParts;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << salvagedParts << std::endl;
		std::cout.flush();

		__timer_start();

		TIEFighterAssembly __object;
		int __actual = __object.assemble(salvagedParts);
