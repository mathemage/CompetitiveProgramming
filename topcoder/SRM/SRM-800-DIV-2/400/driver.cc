...
vector <int>	steps;
		...
		__in >> __expected >> reels >> steps;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << reels << ", " << steps << std::endl;
		std::cout.flush();

		__timer_start();

		SlotMachineHacking __object;
		int __actual = __object.win(reels, steps);
