...
int main(int argc, char* argv[])
{
	...
	for(;;)
	{
		...
		__in >> __expected >> swamp;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << swamp << std::endl;
		std::cout.flush();

		__timer_start();

		PoisonedSwamp __object;
		string __actual = __object.cross(swamp);
