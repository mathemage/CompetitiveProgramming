/* ========================================
   ...
   * Last Modified : Fri 15 Oct 2021 04:39:11 PM CEST
   * Created By : Karel Ha <mathemage@gmail.com>
   * URL : https://codingcompetitions.withgoogle.com/kickstart/round/00000000001a0069/0000000000414a24
   * Points/Time :
   * ~4m 
   *
   * Total/ETA : 8+12+17 pts
   * Status :
   * [also attempts at TS3]
   * S AC AC TLE :-/
   *
   ==========================================*/
...
void solve() {
  ll W,N; cin >> W >> N;
  vector<ll> X(W); cin >> X;
  MSG(N); MSG(X); LINESEP1;

  unordered_map<ll, ll> val2cnt;
  for (auto & Xi: X) {
    val2cnt[Xi]++;
  }

  ll result = INF_LL;
  for (auto & [v, _]: val2cnt) {
    for (auto & target: {v, 1+(v-1+N/2)%N }) {
      MSG(target);

      ll cost=0;
      for (auto & [val, cnt]: val2cnt) {
        ll delta = min(abs(target-val), N-abs(target-val));
        cost += cnt * delta;
      }
      MSG(cost);
