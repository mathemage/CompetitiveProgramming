/* ========================================
   ...
   * Last Modified : Sat 18 Sep 2021 12:12:38 AM CEST
...
//   Rb--, Pb--;
  ...
  function<ll(ll, ll, ll, ll, char, bool, bool)> search = [&](ll curRa, ll curPa, ll curRb, ll curPb, char onMove, bool stopA, bool stopB) {
    LINESEP1;
    MSG_VEC_VEC(grid);
    ...
    for (auto & [nr, np]: neighbors[r][p]) {
      if (grid[nr][np]=='.') {
        ...
        if (onMove=='A') {
          scores.PB(search(nr, np, curRb, curPb, 'B', false, stopB));
        } else {
          scores.PB(search(curRa, curPa, nr, np, 'A', stopA, false));
        }
  ...
  cout << search(Ra, Pa, Rb, Pb, 'A', false, false) << endl;
}
