/* ========================================
   ...
   * Last Modified : Sat 18 Sep 2021 12:25:11 AM CEST
   * Created By : Karel Ha <mathemage@gmail.com>
   * URL : https://codingcompetitions.withgoogle.com/kickstart/round/000000000019ff48/00000000003f47fb
   * Points/Time :
   * +24m
   * +39m = 1h 3m
   *
...
void solve() {
  ll S,Ra,Pa,Rb,Pb,C; cin >> S >> Ra >> Pa >> Rb >> Pb >> C;
  vector<vector<char>> grid(S+1, vector<char>(2*S, '.'));
...
//   Rb--, Pb--;
  ...
  unordered_map<ll, unordered_map<ll,vector<pair<ll, ll>>>> neighbors;
  FOR(r,1,S) {
    FOR(p,1,2*r-1) {
      if (p-1>=1) {
        neighbors[r][p].PB({r,p-1});
      }
      if (p+1<=2*r-1) {
        neighbors[r][p].PB({r,p+1});
      }
  ...
  function<ll(ll, ll, ll, ll, char, ll)> search = [&](ll curRa, ll curPa, ll curRb, ll curPb, char onMove, ll prevBranchFactor) {
    LINESEP1;
    MSG_VEC_VEC(grid);

    ll r = onMove=='A' ? curRa : curRb;
    ll p = onMove=='A' ? curPa : curPb;
    
    ll branchFactor=0;
    for (auto & [nr, np]: neighbors[r][p]) {
      if (grid[nr][np]=='.') {
        branchFactor++;
      }
    }
    ...
    for (auto & [nr, np]: neighbors[r][p]) {
      if (grid[nr][np]=='.') {
        ...
        if (onMove=='A') {
          scores.PB(search(nr, np, curRb, curPb, 'B', branchFactor));
        } else {
          scores.PB(search(curRa, curPa, nr, np, 'A', branchFactor));
        }
    ...
    if (branchFactor==0) {
      if (prevBranchFactor==0) {  // leaf state
        ll result = 0LL;
        for (auto & row: grid) {
          for (auto & cell: row) {
            if (cell=='A') {
        ...
        return search(curRa, curPa, curRb, curPb, onMove^'A'^'B', branchFactor);
      }
    } else {
      return onMove=='A' ? *max_element(ALL(scores)) : *min_element(ALL(scores));
    }
  };

  cout << search(Ra, Pa, Rb, Pb, 'A', UNDEF) << endl;
}
