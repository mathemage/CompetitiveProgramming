/* ========================================
   ...
   * Last Modified : Fri 21 May 2021 12:14:26 AM CEST
...
void solve() {
  ...
  unordered_map<ll, ll> cntOfPrefSums;
  cntOfPrefSums[0]=1;     // empty prefix

  ll prefSum=0LL;
  ll minPrefSum=prefSum;
  REP(i,N) {
    LINESEP1;

    cin >> A[i];
    prefSum+=A[i];
    MSG(A[i]); MSG(prefSum); LINESEP1;
    ...
    for (auto & sq: squares) {
      ...
      if (CONTAINS(cntOfPrefSums, need)) {
        ...
        MSG(sq); MSG(need);
        MSG(cntOfPrefSums[need]); MSG(result);
        LINESEP1;
      }
    }

    cntOfPrefSums[prefSum]++;
    MINUPDATE(minPrefSum, prefSum);
  }
  MSG(A); LINESEP1;
