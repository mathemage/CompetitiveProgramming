/* ========================================
   ...
   * Last Modified : Thu 15 Apr 2021 04:24:00 PM CEST
...
void solve() {
  ...
  // TODO MINUPDATE(span of hole, 350);
  
  MINUPDATE(W, R+1);
  MINUPDATE(H, D+1);

  int nRows=1+H, nCols=1+W;
  const int colTimeout=350;
  MINUPDATE(nCols, colTimeout+(R-L+1)+1);
  vector<double> dp(nCols);
  dp[W]=1.0;

  double nNeigh,sum;
  int rowTimer=0;
//   const int rowTimeout=1000;
//   const int rowTimeout=100;
  const int rowTimeout=350;
  FORD(r,H,1) {
//     FORD(c,W,1) {
    FORD(c, W-0, W-(nCols-1)) {
      if (U<=r && r<=D && L<=c && c<=R) {
        c=L; // TODO
        dp[c] = 0.0;
        continue;
...
//           skip=true;
      ...
      if (nNeigh) { dp[c]=sum/nNeigh; }
//       if (skip) { c=L; } // TODO
    }

    MSG(r); MSG_VEC_VEC(dp);

    // optimize rows
    if (++rowTimer>rowTimeout) {
      if (r>U) {
        r=U;
      } else {
        MINUPDATE(r, rowTimeout);
      }
      rowTimer=0;
    }
