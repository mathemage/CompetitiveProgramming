/* ========================================
   ...
   * Last Modified : Sun 22 May 2022 01:11:20 PM CEST
...
void solve() {
  ...
  if (!std::accumulate(ALL(pw), true, [&](const auto & s, const auto & x) { return s && 'A'<=x && x<='Z'; } )) {
    result += 'A';
  }
  if (!std::accumulate(ALL(pw), true, [&](const auto & s, const auto & x) { return s && 'a'<=x && x<='z'; } )) {
    result += 'a';
  }
  if (!std::accumulate(ALL(pw), true, [&](const auto & s, const auto & x) { return s && '0'<=x && x<='9'; } )) {
    result += '0';
  }
  string spec = "#@*&";
  if (!std::accumulate(ALL(pw), true, [&](const auto & s, const auto & x) { return s && std::count(ALL(spec), x)>0; } )) {
    result += '#';
  }
