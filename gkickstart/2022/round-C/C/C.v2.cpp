...
void solve() {
  ...
  vector<ll> result;
  vector<bool> fallenOff(N);
  while (SZ(result)<N) {
    priority_queue<ll, vector<ll>, greater<ll>> q;
    vector<vector<ll>> newRuler(L+5);
    vector<ll> newD = D;

    REP(i,N) {
      if (fallenOff[i]) { continue; }

      ll dir = D[i]==1LL ? 1LL : (-1LL);
      ll newP = P[i] + dir;
      ll nxtNxtP = newP + dir;
      ...
      } else if (1<=nxtNxtP && nxtNxtP<=L-1 && !ruler[nxtNxtP].empty()) {
        for (auto & ne: ruler[nxtNxtP]) {
          if (D[ne]!=D[i]) {
            newD[i]^=1;
            P[i] = newP;
            newRuler[newP].PB(i);
