/* ========================================
   ...
   * Last Modified : Sun 22 May 2022 02:42:16 PM CEST
...
void solve() {
  ...
  vector<ll> idx(N); std::iota(ALL(idx), 0);
  do {
    vector<bool> used(N);
    for (auto & i: idx) {
      used[i]=1;
  ...
  ll result = candies * mpow(total, MOD-2LL);
  result %= MOD;
  cout << result << endl;
}
