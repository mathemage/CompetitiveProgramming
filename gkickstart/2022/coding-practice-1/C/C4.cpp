/* ========================================
   ...
   * Last Modified : Sat 19 Feb 2022 11:36:40 PM CET
...
void solve() {
  ...
  ll Ci;
  priority_queue<ll, vector<ll>, std::greater_equal<ll>> candidates;
  REP(_,N) {
    cin >> Ci;
    ...
    while (!candidates.empty() && candidates.top()<H+1) {
      candidates.pop();
    }

    if (SZ(candidates)>=H+1) {
      H++;
    }
