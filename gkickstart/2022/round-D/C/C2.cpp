/* ========================================
   ...
   * Last Modified : Mon 11 Jul 2022 08:50:44 PM CEST
   * Created By : Karel Ha <mathemage@gmail.com>
   * URL : https://codingcompetitions.withgoogle.com/kickstart/round/00000000008caea6/0000000000b76f44#problem
   * Points/Time :
   * = 19m
   *
   * Total/ETA : 8+12+10 pts
   * Status :
   * S AC AC TLE!!! ^_^ ^_^
   *
   ==========================================*/
...
void solve() {
  ...
  unordered_map<ll, vector<ll>> ch2pos;
  REP(pos,M) {
    ch2pos[K[pos]].PB(pos);
  }
  MSG(ch2pos); LINESEP2;

//   vector<vector<ll>> dp(N+5, vector<ll>(M+5,INF_LL)); // N is 1-based indexed
//   dp[0].assign(M+5,0LL);
  vector<vector<ll>> dp(N+1, vector<ll>(M,INF_LL)); // N is 1-based indexed
  dp[0].assign(M,0LL);
  MSG_VEC_VEC(dp); LINESEP2;
  ...
  FOR(iS,1,N) {
    ...
    MSG(S[iS-1]);
    MSG(ch2pos[S[iS-1]]);
    for (auto & pos: ch2pos[S[iS-1]]) {
      REP(iK,M) {
        umin(dp[iS][iK], dp[iS-1][pos]+abs(pos-iK));
      }
    }
  ...
  ll result = std::accumulate(ALL(dp[N]), INF_LL, [&](const auto & s, const auto & x) { return min(s, x); } );
  cout << result << endl;
}
