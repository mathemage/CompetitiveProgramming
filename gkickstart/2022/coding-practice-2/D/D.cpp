/* ========================================
   ...
   * Last Modified : Thu 30 Jun 2022 07:28:08 PM CEST
...
void solve() {
  ...
  ll pos=0;
  for (auto & ch: expr) {
    if (std::count(ALL(vowels), ch)) {
      posVowels.PB(pos);
    }

    if (SZ(posVowels)>=5) {
      ll last=SZ(posVowels)-1;

      ll len1=posVowels[last-3]-posVowels[last-4]+1;
      string w1=expr.substr(posVowels[last-4], len1);

      ll len2=posVowels[last]-posVowels[last-1]+1;
      string w2=expr.substr(posVowels[last-1], len1);
      MSG(last);
      MSG(len1); MSG(w1);
      MSG(len2); MSG(w2);
      LINESEP1;
  ...
  MSG(posVowels); LINESEP2;

  cout << ((result)?"Spell!":"Nothing.") << endl;
}
