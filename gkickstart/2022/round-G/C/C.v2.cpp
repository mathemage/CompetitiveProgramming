/* ========================================
   ...
   * Last Modified : Sat 15 Oct 2022 04:54:35 PM CEST
...
void solve() {
  ...
  ll L=0,R=0;   // half-open range [L,R)
  while (L<N && R<N) {
    // phase: skip negatives
    while (L<N && A[L]<0) {
      L++;
    }
    R=L;
    ...
    ll curSum=0LL;
    ll sumAllPrefSum=0LL;
    map<ll, ll> prefSum2Cnt;

    // phase: move R
    while (R<N) {
      if (curSum+A[R]>=0) {
        curSum+=A[R];
        sumAllPrefSum+=curSum;
        prefSum2Cnt[curSum]++;
        R++;
      } else {
        break;
    ...
    while (L<R) {
      ...
      if (prefSum2Cnt.begin()->F - offset >= 0) {
        result += sumAllPrefSum;
      }
      ...
      assert(R-L>=1);
//       assert(R-L>=0);  // TODO
      sumAllPrefSum -= height * A[L];
      if (--prefSum2Cnt[A[L]]<=0) {
        prefSum2Cnt.erase(A[L]);
      }
      offset+=A[L];
      L++;
      LINESEP1;
