...
//   assert(D<=2e5);
  ...
  vector<ll> idDescVAscL(N); std::iota(ALL(idDescVAscL), 0);
  std::stable_sort(ALL(idDescVAscL), [&](const auto & i, const auto & j) { return V[i]>V[j] || (V[i]==V[j] && L[i]<L[j]); } );
  MSG(idDescVAscL);
  LINESEP2;
  ...
  for (auto & id: idDescVAscL) {
    ...
    while (!days2del.empty()) {
      ll day=days2del.front(); days2del.pop();
      assert(capacity[day]==0);
      capacity.erase(day);
    }
    MSG(result);
    MSG(capacity);
