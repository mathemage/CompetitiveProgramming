/* ========================================
   ...
   * Last Modified : Sun 18 Sep 2022 07:15:08 PM CEST
   * Created By : Karel Ha <mathemage@gmail.com>
   * URL : https://codingcompetitions.withgoogle.com/kickstart/round/00000000008cb409/0000000000beefbb
   * Points/Time :
   * Total/ETA :  4+8 pts
   * Status :
   * S AC AC!!!
...
void solve() {
  ...
  vector<ll> ada(N); std::iota(ALL(ada), 0);
  vector<ll> charles(N); std::iota(ALL(charles), 0);

  std::stable_sort(ALL(ada), [&](const auto & i, const auto & j) { return C[i] < C[j] || (C[i] == C[j] && U[i] < U[j]); } );
  std::stable_sort(ALL(charles), [&](const auto & i, const auto & j) { return D[i] < D[j] || (D[i] == D[j] && U[i] < U[j]); } );
