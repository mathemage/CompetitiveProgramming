/* ========================================
   ...
   * Last Modified : Thu 13 May 2021 07:12:45 PM CEST
   * Created By : Karel Ha <mathemage@gmail.com>
   * URL : https://codingcompetitions.withgoogle.com/kickstart/round/0000000000435a5b/000000000077a8e6
   * Points/Time :
   *      = 13m :-O :-/
   * + 2m = 15m
   *
   ...
   *
   * Status :
   * [editorial] <- hardcore number theory (maximal prime gaps, segmented sieves) :-O
   * S WA -  -
   * S AC AC AC (finally passed TS3 !!)
   *
   ==========================================*/
...
ll Z;
const ll MAX_GAP=282;

inline bool is_prime(ll k) {
  if (k%2==0) {
    return k==2;
  }

  for (ll d = 3; d*d <= k; d += 2) {
    if (k%d==0) {
      return false;
    }
  }
...
void solve() {
  ...
  ll result = 0LL;
  ll lastPrime=UNDEF;
  FOR(p, max(0LL, ZZ-2*MAX_GAP-1), ZZ+MAX_GAP+1) {
    if (is_prime(p)) {
      if (lastPrime!=UNDEF && p*lastPrime<=Z) {
        MAXUPDATE(result, p*lastPrime);
      }
      lastPrime=p;
    }
  }
