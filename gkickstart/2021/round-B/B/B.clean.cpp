...
void solve() {
  ...
  vector<vector<ll>> len(2, vector<ll>(N,0));
  FOR(i,1,N-1) {
    if (i>1 && diff[FW][i] == diff[FW][i-1]) {
      len[FW][i]=len[FW][i-1]+1;
    } else {
      len[FW][i]=2;
    }
    MAXUPDATE(result, len[FW][i]);
  }
  FORD(i,N-2,0) {
    if (i<N-2 && diff[BW][i] == diff[BW][i+1]) {
      len[BW][i]=len[BW][i+1]+1;
    } else {
      len[BW][i]=2;
    }
