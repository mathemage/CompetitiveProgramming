/* ========================================
   ...
   * Last Modified : Mon 19 Apr 2021 11:52:53 PM CEST
...
void solve() {
  ...
  REP(i,N) {
    ...
    if (fwdNeed==bwdNeed) {
      ...
      if (fwdNeed!=UNDEF) {
        if (i+1<N && A[i+1]-fwdNeed==fwdNeed-A[i-1]) {
          MAXUPDATE(result,fwdLen+2);
        } else {
      ...
      if (bwdNeed!=UNDEF) {
        if (i-1>=0 && A[i+1]-bwdNeed==bwdNeed-A[i-1]) {
          MAXUPDATE(result,bwdLen+2);
        } else {
