/* ========================================
   ...
   * Last Modified : Sun 22 Aug 2021 08:18:02 AM CEST
...
void solve() {
  ll N,M; cin >> N >> M;
  vector<string> R(N); cin >> R;
  MSG(R);
  LINESEP1;

  vector<vector<int>> mirC(N, vector<int>(M,UNDEF));
  REP(row,N) {
    ll prev=-1;
    REP(col,M+1) {
      if (col==M || R[row][col]=='#') {
        // fill mirrored coordinates
        ll l=prev+1, r=col-1;
        while (l<=r) {
          mirC[row][l]=r;
          mirC[row][r]=l;
  ...
  MSG_VEC_VEC(mirC); LINESEP1;

  vector<vector<int>> mirR(N, vector<int>(M,UNDEF));
  REP(col,M) {
    ll prev=-1;
    REP(row,N+1) {
      if (row==N || R[row][col]=='#') {
  ...
  MSG_VEC_VEC(mirR); LINESEP1;

  queue<ll> qr;
  queue<ll> qc;
  REP(row,N) {
    REP(col,M) {
      if (isupper(R[row][col])) {
        qr.push(row);
        qc.push(col);
      }
    }
  ...
  while (!qr.empty()) {
    ll r=qr.front(); qr.pop();
    ll c=qc.front(); qc.pop();

    ll rm=mirR[r][c];
    if (result[rm][c]=='.') {
      result[rm][c]=result[r][c];
      nFilled++;
