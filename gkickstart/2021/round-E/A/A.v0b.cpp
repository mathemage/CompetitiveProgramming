/* ========================================
   ...
   * Last Modified : Sun 22 Aug 2021 05:52:45 AM CEST
...
void solve() {
  ...
  int n=SZ(S);
  unordered_map<char, vector<bool>> posUsed;
  REP(i,n) {
    posUsed[S[i]].reserve(n);
  }
  ...
  REP(i,n) {
    char ch=S[i];
    while (next[ch]<n && (posUsed[ch][next[ch]] || used[next[ch]])) {
      next[ch]++;
    }
