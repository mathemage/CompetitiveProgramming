/* ========================================
   ...
   * Last Modified : Thu 26 Aug 2021 08:15:56 PM CEST
...
void solve() {
  ...
  unordered_map<char, vector<ll>> ch2positions;
  ll n=SZ(S);
  REP(i,n) {
    ch2positions[S[i]].PB(i);
  }

  set<pair<ll, char>, std::greater<pair<ll, char>>> piles;
  for (auto & [ch, positions]: ch2positions) {
    MSG(ch); MSG(positions);

    if (SZ(positions)>n-SZ(positions)) {
      cout << "IMPOSSIBLE" << endl;
      return;
    }

    piles.insert({SZ(positions),ch});
    LINESEP1;
  }
  MSG(piles); LINESEP1;

  while (SZ(piles)>=2) {
    auto it1 = piles.begin();
    ll cnt1 = it1->F;
    char ch1 = it1->S;
    piles.erase(it1);

    auto it2 = piles.begin();
    ll cnt2 = it2->F;
    char ch2 = it2->S;
    piles.erase(it2);

    ll & pos1=ch2positions[ch1][cnt1-1];
    ll & pos2=ch2positions[ch2][cnt2-1];
    swap(A[pos1], A[pos2]);
//     swap(ch2positions[ch1][cnt1-1], ch2positions[ch2][cnt2-1]);
    swap(pos1, pos2);

    if (--cnt1) { piles.insert({cnt1,ch1}); }
    if (--cnt2) { piles.insert({cnt2,ch2}); }

    MSG(piles); LINESEP1;
  }

  if (SZ(piles)) {
    ll cnt = piles.begin()->F;
    char ch = piles.begin()->S;
    assert(cnt==1);

    ll needPos=ch2positions[ch][0];
    bool ok=0;
    REP(pos,n) {
      if (S[pos]!=ch && A[pos]!=ch) {
        swap(A[pos], A[needPos]);
        ok=1;
        break;
      }
    }
