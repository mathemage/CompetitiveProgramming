...
/* ========================================
   ...
   * Last Modified : Pá 9. července 2021, 20:52:57
...
void solve() {
  int R,C; cin >> R >> C;
  vector<vector<int>> grid(R, vector<int>(C)); cin >> grid;
  MSG_VEC_VEC(grid);
  LINESEP1;
  ...
  REP(r,R) {
    REP(c,C) {
      if (r-1>=0 && grid[r][c]) { dp['u'][r][c] += dp['u'][r-1][c]; }
      if (c-1>=0 && grid[r][c]) { dp['l'][r][c] += dp['l'][r][c-1]; }
    }
  }
  MSG_VEC_VEC(dp['u']);
  MSG_VEC_VEC(dp['l']);
  ...
  FORD(r,R-1,0) {
    FORD(c,C-1,0) {
      if (r+1<R && grid[r][c]) { dp['d'][r][c] += dp['d'][r+1][c]; }
      if (c+1<C && grid[r][c]) { dp['r'][r][c] += dp['r'][r][c+1]; }
    }
  }
  MSG_VEC_VEC(dp['d']);
  MSG_VEC_VEC(dp['r']);
  ...
  REP(r,R) {
    REP(c,C) {
      REP(di,4) {
        int n1=dp[dues[di]][r][c];
        int n2=dp[dues[(di+1)%4]][r][c];

        result += max(0, min(n1, n2/2)-1);
        result += max(0, min(n2, n1/2)-1);
      }
    }
  }
