/* ========================================
   ...
   * Last Modified : Sat 18 Sep 2021 07:18:34 PM CEST
...
void solve() {
  ...
  REP(pos,N) {
    ...
    auto itH=bins.lower_bound(pos);
    hi = (itH==bins.end()) ? INF : *itH;
    --itH;
    lo = (itH==bins.end()) ? -INF : *itH;

    MSG(hi); MSG(lo);
//     assert(hi>=pos);

    result += min(abs(hi-pos), abs(pos-lo));
    MSG(result); LINESEP1;
  }
