/* ========================================
   ...
   * Last Modified : Sat 18 Sep 2021 09:24:19 PM CEST
...
void solve() {
  ...
  for (auto & [pt, id]: pts2id) {
    ...
    if (pt==s[id]) {
      ...
      if (SZ(availHapId)>K) { // TODO possible bad logic here
        if (h[id]>h[idKth]) { // TODO cmp id's too?
          Ksum-=h[idKth];
