/* ========================================
   ...
   * Last Modified : Sat 18 Sep 2021 07:45:45 PM CEST
...
void solve() {
  ...
  FOR(pos,*min_element(ALL(s)),*max_element(ALL(e))) {
    vector<ll> happies;
    REP(i,N) {
      if (s[i]<=pos && pos<=e[i]) {
        happies.PB(h[i]);
    ...
    std::sort(REVALL(happies));
    ll cur=0;
    REP(i,min(K, SZ(happies))) {
      cur+=happies[i];
    }
