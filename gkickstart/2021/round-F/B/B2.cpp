/* ========================================
   ...
   * Last Modified : Fri 08 Oct 2021 11:00:16 PM CEST
...
void solve() {
  ...
  for (auto & [pt, ids]: pts2ids) {
    ...
    for (auto & id: closings) {
      ...
      MSG(del); MSG(itDel->F); MSG(itDel->S);
      MSG(itDel==topK.end());

      if (itDel!=topK.end()) { // remove from topK
        sumTopK-=itDel->F;
        topK.erase(itDel);

        while (SZ(topK)<K && SZ(bot)>0) {
          auto itMov = bot.begin();
          topK.insert(*itMov);
          sumTopK+=itMov->F;
          bot.erase(itMov);
        }
      } else {                 // remove from bot
        auto it = bot.find(del);
        assert(it!=bot.end());
        bot.erase(it);
      }
    }
