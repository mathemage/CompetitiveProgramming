/* ========================================
   ...
   * Last Modified : Sat 18 Sep 2021 09:49:12 PM CEST
...
void solve() {
  ...
  unordered_map<ll, ll> pts2id; // TODO what if interval meet at the some point -> multiset??
  ...
  set<pair<ll, ll>> availHapId;
  ll idKth=UNDEF;
  ll Ksum=0;
#warning TODO Kcnt
  ll Kcnt=0;
  for (auto & [pt, id]: pts2id) {
    if (idKth==UNDEF) {
      idKth=id;

//       availHapId.insert({h[id], id});
//       Ksum+=h[id];
//       Kcnt++;
// 
//       umax(result, Ksum);
//       continue;
    ...
    if (pt==s[id]) {
      ...
      if (Kcnt>K) {
        if (h[id]>h[idKth] || (h[id]==h[idKth] && id<idKth)) {
          Ksum-=h[idKth];
          Kcnt--;

          assert(CONTAINS(availHapId, MP(h[idKth], idKth)));
          auto it=availHapId.find({h[idKth], idKth});
          --it;
          idKth=it->S;
        }
      }
