/* ========================================
   ...
   * Last Modified : Sat 18 Sep 2021 08:54:37 PM CEST
   ...
   * Status :
   * S AC RE!! yes!
   * S AC TLE  ok, expected, but at least tried...
   *
   ==========================================*/
...
//   assert(N<=8);
  ...
  do {
    ...
    curSol.PB(V[0]);
    if (curPts==K) {
      solutions.insert(curSol);
    }
    
    FOR(i,1,N-1) {
      curSol.PB(V[i]);
      if ( !(L[V[i]]<=curPts && curPts<=R[V[i]]) ) {
        break;
      }
