/* ========================================
   ...
   * Last Modified : Sat 18 Sep 2021 08:03:54 PM CEST
...
inline ld area(ll x1, ll y1, ll x2, ll y2, ll x3, ll y3) {
...
bool onBorder(ll x1, ll y1, ll x2, ll y2, ll x3, ll y3, ll x, ll y) {
  ld A1 = area (x, y, x2, y2, x3, y3);
  ld A2 = area (x1, y1, x, y, x3, y3);
  ld A3 = area (x1, y1, x2, y2, x, y);
...
inline ld dist(ll x1, ll y1, ll x2, ll y2) {
    ...
    FOR(j,i+1,N-1) {
      FOR(k,j+1,N-1) {
        if (isInside(X[i], Y[i], X[j], Y[j], X[k], Y[k], Xs, Ys)
         && !onBorder(X[i], Y[i], X[j], Y[j], X[k], Y[k], Xs, Ys)) {
