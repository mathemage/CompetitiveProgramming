/* ========================================
   ...
   * Points/Time :
   * Total/ETA : 11+15pts (now only for 11pts)
   * Status :
   * S AC RE!!
   *
   ==========================================*/
...
void solve() {
  ...
  avail.resize(maxB+1);
  MSG(SZ(avail)); MSG(avail); LINESEP1;
...
#warning Remove the assert in TS2
    assert(!(lo==UNDEF && hi==UNDEF));

    if (lo==UNDEF) { P[j]=hi; }
    else if (hi==UNDEF) { P[j]=lo; }
    else if (S[j]-lo==hi-S[j]) { P[j]=lo; }
    else  { P[j] = (S[j]-lo<hi-S[j]) ? lo : hi; }
