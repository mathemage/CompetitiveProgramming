...
void solve() {
  ...
  MSG(AB); LINESEP1;
  MSG(SZ(L)); MSG(SZ(R)); LINESEP1;

  vector<long long> P(M,UNDEF);

  vector<long long> S(M);
  REP(j,M) {
    cin >> S[j];

#warning TODO inside interval
//     ll Al=*(upper_bound(ALL(L), S[j]));
//     ll Bl=*(lower_bound(ALL(R), S[j]));
//     if (lo<=hi) {
// 
//     } else {
// 
...
#warning TODO outside interval
    ll lo=*(--upper_bound(ALL(R), S[j]));
    ll hi=*lower_bound(ALL(L), S[j]);
