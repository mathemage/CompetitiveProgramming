/* ========================================
   ...
   * Last Modified : Sun 11 Jul 2021 09:47:59 AM CEST
...
//   ll PP=1LL;   // TODO optimize via PP-step sieve
  FOR(x,2,1000) {
    ll xx=x;
    while (xx>1 && xx%P==0) {
      xx/=P;
  ...
  FOR(S,1,4) {
    ...
    FOR(i,1,N) {
      MSG(i);
      prefSum[S][i] = prefSum[S][i-1] + V[ diffs[S][A[i-1]] ];
    }
    LINESEP1;
  }
...
//     MSG(type);
    ...
    switch (type) {
      case 1: 
        cin >> pos >> val;
//         MSG(pos); MSG(val); LINESEP1;
        break;

      case 2: 
        cin >> S >> L >> R;
//         MSG(S); MSG(L); MSG(R); LINESEP1;
