/* ========================================
   ...
   * Last Modified : Sun 11 Jul 2021 07:30:39 AM CEST
...
bool isProgr(ll a, ll b, ll c) {
  return b-a==c-b;
}

void solve() {
  vector<vector<long long>> G(3, vector<long long>(3));
  G[1].resize(2);
  cin >> G;

  G[1].PB(CLEAN);
  swap(G[1][1], G[1][2]);
  MSG_VEC_VEC(G); LINESEP1;
  ...
  for (auto & r: {0,2}) {
    result+=isProgr(G[r][0], G[r][1], G[r][2]);
  }
  for (auto & c: {0,2}) {
    result+=isProgr(G[0][c], G[1][c], G[2][c]);
  }
