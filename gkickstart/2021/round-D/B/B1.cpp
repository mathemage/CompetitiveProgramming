...
void solve() {
  ...
  sort(ALL(LR), [](auto & a, auto & b) { return a.F<b.F || (a.F==b.F && a.S<b.S); } );
  MSG(LR);
  LINESEP1;
  ...
  FOR(x,lo+1,hi-1) {
    ...
    // TODO improve via {lower,upper}bound
    FOR(i,0,N-1) {
      if (x<=LR[i].F) {
        break;
      }

      nCut+=LR[i].F<x && x<LR[i].S;
    }
    cuts.PB(nCut);
  }
  sort(REVALL(cuts));
  MSG(SZ(cuts)); MSG(cuts); LINESEP1;

  ll result = N;
  REP(i,min(C, ll(SZ(cuts)))) {
    result+=cuts[i];
  }
  cout << result << endl;
}
