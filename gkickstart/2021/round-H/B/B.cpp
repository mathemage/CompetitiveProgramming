/* ========================================
   ...
   * Last Modified : Sun 14 Nov 2021 04:33:02 AM CET
...
void solve() {
  ...
  unordered_map<char, vector<bool>> used;
  for (auto & ch: P) {
    switch (ch) {
      case 'U':
        used['R'].PB(0);
  ...
  ll result = 0LL;
  for (auto & color: {'R', 'Y', 'B'}) {
    if (used[color][0]) {
      result++;
    }

    for (ll i = 1; i < SZ(P); i += 1) {
      if (used[color][i] && !used[color][i-1]) {
        result++;
      }
    }
  }
