/* ========================================
   ...
   * Last Modified : Sun 16 Jan 2022 07:20:34 PM CET
...
void solve() {
  ...
  if (ok) {
    ...
    for (auto & xy: upper) {
      cout << xy.F << " " << xy.S << endl << flush;
      assert(0<=xy.F); assert(xy.F<=1e9);
      assert(0<=xy.S); assert(xy.S<=1e9);
    }
    std::reverse(ALL(lower));
    for (auto & xy: lower) {
      cout << xy.F << " " << xy.S << endl << flush;
      assert(0<=xy.F); assert(xy.F<=1e9);
      assert(0<=xy.S); assert(xy.S<=1e9);
