/* ========================================
   ...
   * Last Modified : Fri 24 Dec 2021 11:59:12 PM CET
   ...
   * [editorial]
   * S RE - :-O :-/
   * S AC AC AC !!! ^_^ Within the Christmas Eve -> Merry Christmas!
   *
   ==========================================*/
...
void solve() {
  ...
  REP(l,N) {
    ...
    FOR(r,l,N-1) {
      postSum += B[r];
      if (bounded(K-postSum, K+1)) {
        umin(result, r-l+1 + sum2minLen[K-postSum]);

//         MSG(l); MSG(r); MSG(postSum); MSG(K-postSum);
//         MSG(r-l+1);
//         MSG(sum2minLen[K-postSum]);
//         MSG(r-l+1 + sum2minLen[K-postSum]);
//         LINESEP1;
      }
    }
    ...
    FORD(j,l,0) {
      preSum += B[j];
      if (bounded(preSum, K+1)) {
        umin(sum2minLen[preSum], l-j+1);
      }
    }
  }
  MSG(sum2minLen);
