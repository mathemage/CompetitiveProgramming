/* ========================================
   ...
   * File Name : C.TS1-2.cpp
   * Creation Date : 16-10-2021
   * Last Modified : Sat 16 Oct 2021 03:31:02 PM CEST
   * Created By : Karel Ha <mathemage@gmail.com>
   * URL : https://codingcompetitions.withgoogle.com/kickstart/round/00000000004362d6/00000000008b44ef
   * Points/Time :
   * Total/ETA : 8+11+14 pts
   * Status :
   * S AC WA - :-/ ^_^
   *
   ==========================================*/

#define PROBLEMNAME "C.TS1-2"
...
void solve() {
  ll N,K; cin >> N >> K;
  vector<ll> B(N); cin >> B;
  MSG(K); MSG(B); LINESEP1;

  vector<ll> prefSum=B;
  FOR(i,1,N-1) {
    prefSum[i]+=prefSum[i-1];
  }
  MSG(prefSum); LINESEP1;

  unordered_map<ll, vector<pair<ll, ll>>> sum2intervals;
  REP(i,N) {
    FOR(j,i,N-1) {
      ll s=prefSum[j]-(i-1>=0 ? prefSum[i-1] : 0);
      if (s>0) {
        sum2intervals[s].PB({i,j});
      }
//       MSG(i); MSG(j);
//       assert(s>=0);
    }
  ...
  ll result = INF_LL;
  for (auto & [s, intervals]: sum2intervals) {
    std::stable_sort(ALL(intervals), [&](const auto & a, const auto & b) {
        return a.CL-a.OP < b.CL-b.OP;
    } );

    if (s==K) {
      umin(result, intervals[0].CL-intervals[0].OP+1);
    }
  }
  MSG(sum2intervals); LINESEP1;

  for (auto & [s, intervals]: sum2intervals) {
    if (s>K) {
      for (auto & outer: intervals) {
        if (CONTAINS(sum2intervals, s-K)) {
          for (auto & inner: sum2intervals[s-K]) {
            if (outer.OP<inner.OP && inner.CL<outer.CL) {
              umin(result, outer.CL-outer.OP+1 - (inner.CL-inner.OP+1));
              break;
            }
          }
        }
  ...
  cout << (result==INF_LL ? -1: result) << endl;
}
