/* ========================================
   ...
   * Last Modified : Sat 16 Oct 2021 02:46:01 PM CEST
...
void solve() {
  ll K; cin >> K;
#warning TODO remove assert on TS2
  assert(K<=20);

  vector<vector<ll>> x(K, vector<ll>(2));
  vector<vector<ll>> y(K, vector<ll>(2));
  REP(i,K) {
  ...
  FOR(xx,-100,100) {
    FOR(yy,-100,100) {
      ll cost=0;
      REP(i,K) {
        ll dx = min({abs(x[i][0]-xx), abs(x[i][1]-xx)});
        if (x[i][0]<=xx && xx<=x[i][1]) {
          dx=0;
        }

        ll dy = min({abs(y[i][0]-yy), abs(y[i][1]-yy)});
        if (y[i][0]<=yy && yy<=y[i][1]) {
          dy=0;
        }
