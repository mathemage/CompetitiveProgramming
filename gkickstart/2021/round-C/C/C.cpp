/* ========================================
   ...
   * Last Modified : Sun 23 May 2021 04:10:19 PM CEST
...
void solve() {
  ...
  unordered_map<char, ld> cnt;
  unordered_map<char, ld> Pr;
  string result;

  std::random_device rd;  //Will be used to obtain a seed for the random number engine
  std::mt19937 gen(rd()); //Standard mersenne_twister_engine seeded with rd()
  std::uniform_int_distribution<> distribInit(0, 2);

  char choice=RPS[distribInit(gen)];
  result+=choice;
  cnt[choice]++;

//   const double likelyVsSampledFactor=.9;
  const double likelyVsSampledFactor=1.0;
  ...
  FOR(i,2,60) {     // round
    ...
    vector<double> probs;
    for (char ch: RPS) {
      Pr[counter[ch]] = cnt[ch] / (i-1.0);

      if (Pr[counter[ch]]>maxPr) {
        maxPr=Pr[counter[ch]];
        likelyChoice=counter[ch];
      }

      probs.PB(Pr[counter[ch]]);
    }
    MSG(probs);
    assert(likelyChoice!='?');

    std::discrete_distribution<int> distrib{probs[0], probs[1], probs[2]};
    int iChoice=distrib(gen);
    char sampledChoice=RPS[iChoice];
    MSG(distrib(gen)); MSG(iChoice);

    std::discrete_distribution<int> pick{likelyVsSampledFactor, 1.0-likelyVsSampledFactor};
    choice=(pick(gen)==0 ? likelyChoice : sampledChoice);

//     std::discrete_distribution<int> pickEqual{W, E};
    std::discrete_distribution<int> pickEqual{1.0-E/W, E/W};
    char counterChoice=(pickEqual(gen)==0 ? counter[choice] : choice);

//     double diffWE=abs(W-E);
//     if (diffWE<1e-6) {
//       counterChoice=choice;
//     }

    result+=counterChoice;
    cnt[counterChoice]++;
  }
