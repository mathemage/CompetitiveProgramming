/* ========================================
   ...
   ID: mathema6
   TASK: staticRangeMinimumQueries2
   LANG: C++14

   * File Name : staticRangeMinimumQueries2.cpp
   * Creation Date : 09-09-2021
   * Last Modified : Thu 09 Sep 2021 11:18:52 PM CEST
...
#define PROBLEMNAME "staticRangeMinimumQueries2"
...
void solve() {
  ...
  sparseTable.PB(x);
  for (ll exp = 1; 1<<exp <= n; exp += 1) {
    sparseTable.PB({});
    ll len=1<<exp;
//     MSG(exp); MSG(len);

    for (ll pos = 0; pos+len-1 < n; pos += 1) {
//       MSG(pos); MSG(pos+len/2);
//       MSG(sparseTable[exp-1][pos]); MSG(sparseTable[exp-1][pos+len/2]);
      sparseTable[exp].PB(min(sparseTable[exp-1][pos], sparseTable[exp-1][pos+len/2]));
    }
//     LINESEP1;
  }
  MSG_VEC_VEC(sparseTable); LINESEP1;
  ...
  REP(_,q) {
    ...
    ll exp=63-__builtin_clzll(b-a+1);
    MSG(b-a+1); MSG(exp);

    MSG(sparseTable[exp][a]) MSG(sparseTable[exp][b-(1<<exp)+1]);
    ll result = min(sparseTable[exp][a], sparseTable[exp][b-(1<<exp)+1]);
    cout << result << endl;
    LINESEP1;
  }
}
